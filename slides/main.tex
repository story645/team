...
\begin{equation*}
    \begin{tikzcd}[ampersand replacement=\&]
      \& {} \arrow[dd, "\texttt{artist}" description, Rightarrow, shift left=3.65, shorten <= .7em, shorten >= .7em, color=white, font=\Huge]
      \& \\ {\scriptstyle\textcolor{base}{\texttt{topology}}}
        \arrow[rr, "\texttt{graphic}"', bend right, color=white, font=\huge] \arrow[rr, "\texttt{data}", bend left, color=sheaf, font=\huge] \&
...
\begin{equation*}
  ...
  \end{equation*}
    ...
    \begin{itemize}
        \uncover<1-3>{\item uniform API for arbitrary datasets in arbitrary data containers}
        \uncover<2-3>{\item generalizable methodology for expressing structure preservation}
        \uncover<3-3>{\item framework that translates into coding specifications}
    \end{itemize}
\end{frame}
...
\begin{frame}{Structure Preservation: Groups}
\only<2>{
   \begin{figure}
        \includegraphics[width=\linewidth]{../paper/figures/phi.pdf}
        \label{fig:related-work:actions}
      \end{figure}
}
\begin{definition}\label{def:related-work:action}\cite{grimaldiDiscreteCombinatorialMathematics2006}
    An \textcolor{action}{\textbf{action}} of \textcolor{action}{$G = (G,\circ, e)$} on $X$ is a function  $\dfuncc: \textcolor{action}{G} \times X \rightarrow X$. An action has the properties of identity $act(\textcolor{action}{e}, x) = x$ for all  $x \in X$ and associativity $\dfuncc(\textcolor{action}{g}, \dfuncc(\textcolor{action}{f}, x)) = \dfuncc(\textcolor{action}{f} \circ \textcolor{action}{g}, x)$ for $\textcolor{action}{f},\textcolor{action}{g} \in \textcolor{action}{G}$.
\end{definition}
\end{frame}
...
\only<2>{
   \begin{figure}
    ...
    \begin{definition}\label{def:equivariance}
        Given a group $G$ that acts on both the input $X$ and the output $Y$ of a function $f: X \rightarrow Y$, a function $f$ is \textbf{equivariant} when $f(act(g,x)) = act(g,f(x))$ for all $g$ in $G$ and for all $x$ in $X$ \cite{pittsNominalSetsNames2013}
    \end{definition}
\end{frame}
...
\only<2>{
    ...
    \begin{definition}\label{def:homomorphism}
     Given the function $f: X \rightarrow Y$, with operators $(X, \circ)$ and $(Y, *)$, a function $f$ is \textbf{homomorphic} when $f(x_1 \circ x_2) = f(x_1) * f(x_2)$ and preserves identities $f(I_x) = I_y$ all $x, y \in X$ \cite{grimaldiDiscreteCombinatorialMathematics2006}
    \end{definition}
\end{frame}
...
\only<2>{
    \begin{figure}
        \includegraphics[scale=.95]{../paper/figures/k_different_types.pdf}
    \end{figure}
}
        \begin{definition}
...
\begin{frame}{Base \texttt{TYPE} \dbasec}
  \only<1>{
     ...
     \begin{enumerate}
       ...
       \only<2>{
            The base space category consists of the openset object $\dbasec$ and morphisms between openset objects $\dfunchc \in Hom(\dbasec, \dbasec)$.

    The base space category $\mathcal{\dbasec}$ is also equipped with functor  $\oplus: \mathcal{\dbasec} \sqcup \mathcal{\dbasec} \rightarrow \mathcal{\dbasec}$ for combining base spaces. 
       }
\end{frame}
...
\only<3-4>{
    A \textcolor{fiber}{\textbf{record}} is a function $\delement: \fnames \rightarrow \mathscr{U}_{\sigma}$ and the set of records on $\pi_{\sigma}$ is denoted $\Gamma^{\pi}(\sigma)$. Records must return an object of type $\sigma(\fname) \in \textbf{DT}$ for each field $c \in C$.
}

\only<4>{
    \textcolor{section}{Tables} are sections $\dsection: \dbase \rightarrow \Gamma^{\pi}(\sigma)$ from an indexing space $\dbase$ to the set of all possible records $\Gamma^{\pi}(\sigma)$ on the schema bundle
}
\only<5>{
...
\begin{frame}{Fiber category \dfiberc}
    ...
    The fiber category $\mathcal{\dfiberc}$ is also equipped with a bifunctor  $\otimes: \mathcal{\dfiberc} \times \mathcal{\dfiberc} \rightarrow \mathcal{\dfiberc}$ for combining fiber types.
\end{frame}
...
\end{frame}
\begin{frame}{Container invariance: sheaf}
       A \textbf{sheaf} is a presheaf that satisfies the following two axioms  \cite{bakerEuclideanSpaceMathsSheaf,spanier1989algebraic}
    ...
    \begin{columns}
      \column{.6\textwidth}
        ...
        \end{itemize}
           ...
           \only<2>{
            \begin{figure}
                \includegraphics[width=\linewidth]{../paper/figures/tex/local.pdf}
            \end{figure}
           }
             \only<3>{
            \begin{figure}
                \includegraphics[width=\linewidth]{../paper/figures/tex/glue.pdf}
            \end{figure}
           }
             \only<4>{
            \begin{figure}
                \includegraphics[width=\linewidth]{../paper/figures/tex/local_glue.pdf}
            \end{figure}
           }
      \end{columns}
\end{frame}
...
\begin{frame}[Sheaves: Data Containers]
\begin{table}[H]
  \centering
...
\end{equation*}
    ...
    \begin{definition}[natural transform]\label{def:natural-transform}
...
\begin{frame}{Verify encoder}
    \begin{figure}
        \includegraphics[scale=1, keepaspectratio]{../paper/figures/encoder.pdf}
    \end{figure}
\end{frame}
