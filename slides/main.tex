...
\begin{frame}{Presheaf: $\sheafc: \opensetc \textcolor{sheaf}{\rightarrow} \cgamma{\opensetc}{\dtotalc\restriction_{\opensetc}}$}
\begin{equation*}
...
\end{frame}
\begin{frame}{Stalk \& Germ}

\adjustbox{scale=1}{
    \begin{description}
    \item[Stalk] $\sheafc(\dbasec)\restriction_{\dbasepointc}\coloneqq \lim\limits_{\opensetc\ni \dbasepointc} \Gamma(\opensetc, \dtotalc\restriction_{\opensetc})$
    \item[Germ] $\dsectionc(\dbasepointc) \in \sheafc(\dbasec)$
\end{description}
}
\end{frame}
