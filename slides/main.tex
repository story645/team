...
\begin{frame}{Presheaf: $\sheafc: \mathcal{C}^{op} \textcolor{sheaf}{\rightarrow} \setc $}
    ...
    \column{0.69\textwidth}
        \begin{tikzcd}[ampersand replacement = \&, column sep=small]
            ...
            \onslide*<5-|handout:0>{\arrow[uu, "\sheafc_{\dbasec}", maps to, color=sheaf]}
            \onslide*<0|handout:1>{\arrow[uu, "\sheafc_{\dbasec}", maps to, color=sheaf, shift right = 10]}
            \onslide*<7-|handout:1>{\arrow[rr, "\iota"', hook, color=base]} \&   
            \onslide*<8-|handout:1>{\arrow[uu, "\sheafc_{\dbasec}" description, maps to, color=sheaf]} \& 
            \onslide*<6-|handout:1>{\opensetc_2} 
            \onslide*<6-|handout:1>{\arrow[uu, "\sheafc_{\dbasec}"', maps to, color=sheaf]}                   
            \end{tikzcd}
    \end{columns}
    \begin{description}
        \item<8-|handout:1>[stalk]$\sheafc(\dbasec)\restriction_{\dbasepointc}\coloneqq \lim\limits_{\opensetc\ni \dbasepointc} \Gamma(\opensetc, \dtotalc\restriction_{\opensetc}) \approx \dfiberc_{\dbasepointc}$
        \item<9-|handout:1>[germ]   $\dsectionc(\dbasepointc) \in \sheafc_{\dbasec}$
    \end{description}
\end{frame}
...
\begin{frame}{Sheaves}
    ...
    \onslide<2-|handout:1>{Given open cover $\mathcal{\opensetc} = \{\opensetc_i\}_{i\in I}$ of $\opensetc$:}
    \begin{block}<3-|handout:1>{locality}
    ...
    \end{block} 
    \begin{block}<4-|handout:1>{gluing}
...
\begin{frame}{
\only<-8|handout:1->{Morphism of sheaves:}
\only<5-6|handout:0>{pullback}
\only<7|handout:0>{pushforward} 
\only<8,11-|handout:1->{pullback \& pushforward}
\only<9|handout:0>{pullback sections}
\only<10|handout:>{pushforward sections}
}
    \begin{equation*}
        \begin{tikzcd}[ampersand replacement=\&, row sep=huge]
            \onslide*<7-8|handout:1>{\sheafc_{(\gbasec, \gtotalc)}(\gbasec)} 
            \onslide*<10-|handout:2>{\{\gsectionc: \gbasex \scarr \setc \}}
            \onslide*<7-8,10-|handout:1->{\arrow[rr, "\textcolor{functor}{\vindexpush}", color=functor]} \& \& 
            \onslide*<7-8|handout:1>{(\textcolor{functor}{\vindexpush}\sheafc_{(\gbasec, \gtotalc)})(\dbasec)}  
            \onslide*<10-|handout:2>{(\textcolor{functor}{\vindexpush}\gsectionc:\dbasec \scarr \setc)}
            \\
            \onslide*<2-|handout:1->{\gbasec}
            \onslide*<3-|handout:1->{\arrow[rr, "\vindex", color=functor]}
            \onslide*<7-8,10-|handout:1->{\arrow[u, "\sheaf_{\gbasec}", maps to, color=sheaf]}
            \onslide*<6,8-9,11-|handout:1->{\arrow[d, dashed, maps to, color=sheaf]} \&  \& 
            \onslide*<1-|handout:1->{\dbasec}
            \onslide*<4-6,8-9,11-|handout:1->{\arrow[d, "\sheaf_{\dbasec}", maps to, color=sheaf]}
            \onslide*<7-8,10-|handout:1->{\arrow[u, dashed, maps to, color=sheaf]} \\
            \onslide*<5-6,8|handout:1>{(\textcolor{functor}{\vindexpull}\sheafc_{\dbasec})(\gbasec)} 
            \onslide*<9,11-|handout:2>{\{\textcolor{functor}{\vindexpull}\dsectionc:\gbasec \scarr\setc\}}
            \onslide*<5-6,8-9,11-|handout:1->{\arrow[rr, "\textcolor{functor}{\vindexpull}", color=functor]}  \& \& 
            \onslide*<4-6,8|handout:1>{\sheafc_{\dbasec}(\dbasec)}                      
            \onslide*<9,11-|handout:2>{\{\dsectionc: \dbasec \scarr \setc \}}
        \end{tikzcd}
    \end{equation*}
    \onslide<9,11-|handout:2>{
        \begin{equation*}
...
\begin{frame}{
    ...
    \begin{equation*}
        \begin{tikzcd}[ampersand replacement=\&]
          \onslide*<1-4|handout:1-4>{\sheafc_{(\gbasec, \gtotalc)}(\gbasec)} 
          ...
          \& \& 
          \onslide*<1-4|handout:1-4>{(\textcolor{functor}{\vindexpush}\sheafc_{(\gbasec, \gtotalc)})(\dbase)}
          \onslide*<5-|handout:5>{\{\vindexpushc \gsectionc: \dbasepointc \scarr\setc\}}
          \\
          \& \& \\
          \onslide*<1-4|handout:1-4>{(\textcolor{functor}{\vindexpull}\sheafc_{(\dbasec, \dtotalc)})(\gbasec)}
          ...
          \&  \& 
          \onslide*<1-4|handout:1-4>{\sheafc_{(\dbasec, \dtotalc)}(\dbase)} 
    ...
    \onslide<3|handout:3>{
        \begin{equation*}
        \textcolor{homset}{hom}_{\gbasec}(\textcolor{functor}{\vindexpull}\sheafc_{(\dbasec, \dtotalc)}, \sheafc_{(\gbasec, \gtotalc)}) = 
    \textcolor{homset}{hom}_{\dbase}(\sheafc_{(\dbasec, \dtotalc)}, \textcolor{functor}{\vindexpush}\sheafc_{(\gbasec, \gtotalc)})
        \end{equation*}
    }
    \onslide<4|handout:4>{
        \begin{equation*}
...
\begin{frame}{$\vartist_{\vindexpushc}: \sheafc_{\dbasec} \rightarrow \vindexpushc\sheafc_{(\gbasec, \gtotalc)}$}
    \begin{equation}
      \begin{tikzcd}[ampersand replacement =\&]
...
\begin{frame}{$\vartist_{\vindexpullc}: \vindexpullc\sheafc_{\dbasec} \rightarrow \sheafc_{(\gbasec, \gtotalc)}$}
    \begin{equation}
  \begin{tikzcd}[ampersand replacement =\&]
