...
\begin{frame}{Mathematical Data Abstraction}
    \begin{description}
        ...
        %% change SD to Category Theory Langauge - Express structure and constraints [software papers]
...
%drop category K and \gamma op slides
...
\begin{frame}{Presheaf: $\sheafc: \mathcal{C}^{op} \textcolor{sheaf}{\rightarrow} \setc $}
    ...
    \begin{center}
        \begin{alertblock}<8-|handout:1>{stalk}
            % F_k is subset of O_{K,E} is 
            % O_{K,E} is the Jet
            \begin{aligned*}
                \sheafc_{\dbasec, \dtotalc}\restriction_{\dbasepointc}\coloneqq \lim\limits_{\opensetc\ni \dbasepointc} \Gamma(\opensetc, \dtotalc\restriction_{\opensetc}) \ \dfiberc_{\dbasepointc}
            \end{aligned*}
        \end{alertblock}
...
\begin{frame}{Sheaves} %why we care, either drop out or define what an open covering is
    %%% footnote on open covering
    ...
    \end{block} 
    \begin{block}<5-|handout:1>{gluing} %gluing w/o open covering rule
...
%flip the ordering: K, then G, then functor 
\begin{frame}
...
%move other side to when I introduce Q 
...
\begin{frame}{Continuty Equivariance $\vindexpullc\hat\dfuncc = \dfuncc|_{\vindexprec}$}
    \begin{equation*}
        \begin{tikzcd}[ampersand replacement =\&]
            S 
            \arrow[r, "\xi"] 
...
\begin{frame}{Equivariant Artist}
    ...
    \end{equation*}
\begin{block}
    \vartistc\ is equivariant when there exists a $\dfuncc_H$ s.t the diagram commutes. 
    \begin{equation}
