...
\begin{frame}
    ...
    \only<6|handout:2>{
        \begin{description}
            \item[\textcolor{base}{Continuity Type \dbasec}]{how data elements are organized (topological properties) \cite{wilkinsonGrammarGraphics2005,munznerWhatDataAbstraction2014})}
...
\begin{frame}{$\dfuncc = (\dfunchc, \dfuncpullc, \dfunctc)$}
    \begin{tikzcd}[ampersand replacement = \&]
        ...
        \&  \& 
        \cgamma{\opensetc^{\prime}}{\dfunctc\dfuncpullc\dtotalc\restriction_{\opensetc^{\prime}}} \\
        \&  \&  
        \&  \& \\
        \opensetc 
        ...
        \arrow[rruu, "{\dfunctc\dfuncpullc\sheafc_{\dbasec, \dtotalc}}"', maps to, color=sheaf] 
        \&  \& 
    \end{tikzcd}
    \begin{itemize}
        \item Base $\dbasec$ Type stays same: $\dfunchc: \dbasec \rightarrow \dbasec$
        \item Fiber $\dfiberc$ Type stays the same: $\dfunctc: \dfiberc \rightarrow \dfiberc$
...
\begin{frame}{\dfunctc}
    $\dfunctc$ applies a monoid action $\monoidc$ on a fiber set $\dfiberc$
    \begin{equation*}
        \dfunctc: \monoidc \times \dfiberc \rightarrow \dfiberc
    \end{equation*}
    meaning that, by definition of a monoid, the following properties are satisfied
    \begin{description}
        \item[identity]: $\dfunctc(\textcolor{monoid}{e}, \delementc) = \delementc, \delementc \in \dfiberc, \textcolor{monoid}{e} \in \monoidc$
        \item[associativity]  $\dfunctc(\textcolor{monoid}{g}, \dfunctc(\textcolor{monoid}{h}, \delementc)) = \dfunctc(\textcolor{monoid}{g}\circ \textcolor{monoid}{h}, \delementc), \textcolor{monoid}{g}, \textcolor{monoid}{h} \in \monoidc, \delementc \in \dfiberc$
    \end{description}
\begin{block}{Multivariate?} 
    ...
    \begin{itemize}
      \item \textit{object} set \dfiberc with actions \monoidc \cite{nlab:monoid}.
      \item \textit{morphisms} $\dfiberc \otimes \dfiberc \rightarrow \dfiberc$, $\monoidc \otimes \monoidc \rightarrow \monoidc$ and unit
    \end{itemize}
\end{block}
\end{frame}

\begin{frame}{Continuity Equivariance: $\dfunchc_{\gbasec}\circ \vindexc = \vindexc \circ \dfunchc_{\dbasec}$}
    \begin{equation*}
        \begin{tikzcd}[ampersand replacement=\&]
        \opensetgc 
