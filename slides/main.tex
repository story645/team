...
\subsection{Artist}
\begin{frame}
    \frametitle<0|handout:1>{Artist: \vartistc: \sheafc_{\dbasec, \dtotalc} \rightarrow \sheafc_{\gbasec, \gotalc}}
...
\end{alertblock}
\end{frame}
\begin{frame}{Remapping Components}
    \begin{block}<>{Functor $\vchannel$}
        $\vchannelc$ is a functor from on fiber categories $\vchannelc: \mathcal{\dfiberc} \rightarrow \mathcal{\vfiberc}$  

    \end{block}
    \begin{equation*}
        \begin{tikzcd}[ampersand replacement=\&]
            \dfiberc 
            \arrow[rr, "\vchannelc_{\dfiberc}"] 
            \arrow[rrdd, "{\vchannelc_{\dfiberc}}^{\prime}"', dotted] 
            \&  \& 
            \vfiberc \coloneqq \dfiberc^{\prime}  
            \arrow[dd, "\vchannelc_{\vfiberc}"] \\
            \&  \& \\
            \&  \& \vfiberc^{\prime}                           
        \end{tikzcd}
    \end{equation*}
