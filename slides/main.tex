...
\begin{frame}{Fiber Bundles}
    \begin{columns}
     ...
     \column{0.49\columnwidth}
        \begin{description}
            \item[\textcolor{base}{Base Space}] $( \dbasec, \mathcal{T})$
            \item[\textcolor{fiber}{Fiber Space}] $\dfiberc_{\dbasepointc} = \pi^{-1}(\dbasepointc)$
            \item[\textcolor{total}{Total Space}]  $\dtotalc = \dbasec \times \dfiberc$
        \end{description}
    \end{columns}
    \pause
    \begin{block}{Sections of the bundle}
        \begin{equation*}
            \label{eq:related-work:continuity:sections}
            \cgamma{\opensetc}{\dtotalc\restriction_{\opensetc}} \coloneqq \big\{\dsectionc: \opensetc\rightarrow \dtotalc\restriction_{\openset} \; \bigm{\vert} \pi(\dsectionc(\dbasepointc)) = \dbasepointc\;\forall \dbasepointc \in \opensetc \subset \dbasec \big\}
        \end{equation*}
    \end{block}
\end{frame}
