...
The graphic category $\mathcal{\gtotal}$ consists of the functor object $\sheaf_{\mathcal{\gtotal}}: \mathcal{S}^{op} \rightarrow \Gamma(\mathcal{S}, \gtotal)$ and morphisms between sheaves. The category $\mathcal{S}$ is the category of open sets $\{U\}$ over the graphic base space \gbase. The set of sets of sections $\Gamma(\mathcal{S}, \gtotal)$ is a subcatagory of \texbf{Set}. A set $\Gamma(U_i, \gtotal)$ contains functions to generate arbitrary graphics with a shared continuity and display space; bar plots and scatter plots on a 2D screen are two such objects. 





As will be discussed in \autoref{sec:artist:construction}, the  $\vartist$ is constructed in such a way that it generates a single type of graphic; 


Therefore, we define the object as a morphism from data to the sheaf mage of the artist in the graphic space $\vartist: \mathcal{\dtotal} \rightarrow Im_A \subset \mathcal{H}$
