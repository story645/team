...
Visualizations generally consist of more than one visual element, for example line plots, a legend, axis ticks, spines, and labels. Many times, visualizations are generated from subsets of a larger data set, such as when using multiple views \cite{alboRadarComparativeEvaluation2016a} and brush-linked views \cite{beckerBrushingScatterplots1987,bujaInteractiveData1991}, and there is an expectation of consistency in how this data is visualized\cite{hullmanKeeping2018}. In this section, we generalize the sheaf input object $\sheaf_{\dtotal}$ to a collection of sheaves $E^\prime$ with inclusion maps that dictate the how the sheaves are glued together. For example, given$\mathcal{E}_1, \mathcal{E}_2 \in \mathcal{E^{prime}}

 (\note{need citation/explainationn of why I can just do this}) 
...
\begin{figure}[!h]
  \centering
  \subfloat[$K_{k}\neq \varnothing$]{\includegraphics[width=.4\columnwidth]{union_k.png}%
  \label{fig:artist:union:k}}
  %\hfil
  \subfloat[$K_{k}=\varnothing$]{\includegraphics[width=.4\columnwidth]{disjoint_k.png}%
...
\begin{tikzcd}
  ...
  \dbase_{i} \arrow[r, "i_{\dbase_i}"]                         & \dbase_{i} \bigsqcup_{\dbase_{k}}\dbase_{j}
  \end{tikzcd}
where the projection functions $i_{\dbase_i},i_{\dbase_j}$ behave such that $i_{\dbase_{i}}(\dbasepoint)|_{\dbasepoint \in \dbase_i} = \[\dbasepoint\]$,  $i_{\dbase_{j}}(\dbasepoint)|_{\dbasepoint \in \dbase_j} = \[\dbasepoint\]$, meaning the projection functions yield equivalent points in the disjoint union $\dbase_i \bigsqcup \dbase_j$. The special case of 
