...
\subsubsection{Data Transformations $\dfuncc=(\dfunchc, \dfunctc)$}
We express transformations of data, such as scaling and rotation, as function on sheaves: 
\note{may not need the diagram here}
\begin{equation}
...
\end{equation}
is used to express actions such as rotation that induce both a remapping of the index space and a change in the data values. The data transform \dfunc\ is composable if there exists functions $\dfunc_{a,b}: \dtotal_a \times \dtotal_b \rightarrow \dtotal_a \times \dtotal_b$, $\dfunc_{a}: \dtotal_a \rightarrow \dtotal_a$ and $\dfunc_{b}: \dtotal_b \rightarrow \dtotal_b$ s.t. $\pi_a \circ \dfunc_a = \dfunc_{a,b} \circ \pi_a$ and $\pi_b \circ \dfunc_b = \dfunc_{a,b} \circ \pi_b$ then $\dfunc_{a,b} = (\dfunc_a, \dfunc_b)$. This allows us to define a data transform $\dfunc = (\dfunch, \prod\limits_{i=0}^{n}\dfunct_i)$ where each fiber transform $\dfunct_{i}$ is applied to a different fiber field $\dfiber_i$. 
...
\begin{equation}
  \begin{tikzcd}[row sep=huge]
    ...
    \arrow[rr, "\vindexpushc", color=functor]   &  & \cgamma{\opensetc}{\vindexpushc\gtotalc\restriction_{\opensetc}}\\
    \opensetgc \arrow[rr, "\vindexc", color=functor] 
    \arrow[u, "{\sheafc_{\gbasec, \gtotalc}}", maps to, color=sheaf] 
    \arrow[d, "{\sheafc_{\gbasec, \vindexpullc\dtotalc}}"', dashed, maps to, color=sheaf] &  & \opensetc 
    \arrow[d, "{\sheafc_{\dbasec,\dtotalc}}", maps to, color=sheaf] 
    \arrow[u, "{\sheafc_{\dbasec, \vindexpushc \gtotalc}}"', dashed, maps to, color=sheaf] \\
    \cgamma{\opensetgc}{\vindexpullc\dtotalc\restriction_{\opensetgc}}  &  & \cgamma{\opensetc}{\dtotalc\restriction_{\opensetc}}
    \arrow[ll, "\vindexpullc"', color=functor]      
    \end{tikzcd}
\end{equation}
...
\begin{tikzcd}
  ...
  \arrow[rr, "\vindexpushc", color=functor]  
  &  & {\sheafc_{\dbasec,\vindexpushc\gtotalc}}   \\
   &  &             \\
  {\sheafc_{\gbasec,\vindexpullc\dtotalc}} 
  \arrow[uu, "\vartistc^{\gbasec}", Rightarrow, color=artist] &  & {\sheafc_{\dbasec,\dtotalc}} 
  \arrow[ll, "\vindexpullc"', color=functor]
   \arrow[uu, "\vartistc^{\dbasec}"', Rightarrow, color=artist] 
   \arrow[lluu, "\vartistc" description, color=artist]
  \end{tikzcd}
\end{equation}
