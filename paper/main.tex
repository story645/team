...
\end{equation}
where $\dsection, \dsection^{\prime} \in \Gamma(\openset^{\prime}, \dfuncpull\dtotal\restriction_{\openset^{\prime}})$. Since there are many different fiber transformations,  there is not a uniform way of expressing how this morphism acts on sections unlike in \autoref{eq:atct:morphism:verify_base}.\note{This is where we reintroduce the equivariance and homomorphism definitions from the introduction}
...
\begin{table}[h!]
  ...
  \begin{tabular}{|lll|}\hline
      scale & property & scale specific constraints \\ \hline
      nominal & permutation &  $\dsectionc(\dbasepointc_1) \neq \dsectionc(\dbasepointc_2) \implies \dfunctc (\dsectionc(\dbasepointc_1)) \neq\dfunctc(\dsectionc(\dbasepointc_2)$)\\
      ordinal &  monotonic &  $\dsectionc(\dbasepointc_1) \leq \dsectionc(\dbasepointc_2) \implies \dfunctc (\dsectionc(\dbasepointc_1)) \leq \dfunctc(\dsectionc(\dbasepointc_2)$) \\
      interval &  translation &  $\dfunctc(\dsectionc(\dbasepointc) + C) = \dfunctc(\dsectionc(\dbasepointc)) + C$ \\
      ratio &  scaling &  $\dfunctc(\dsectionc(\dbasepointc) * C) = \dfunctc(\dsectionc(\dbasepointc))*C $\\ \hline
  \end{tabular}
  \caption{The record transformer $\dfunctc$ is defined by the general constraints listed in \autoref{tab:appendix:summary:datamod} and it must respect any properties that are specific to the record field. For example, this table lists the properties of the Steven's measurement scale types\cite{stevensTheoryScalesMeasurement1946}}
  \label{tab:appendix:summary:datatypes}
\end{table}
