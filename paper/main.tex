...
\begin{definition}\cite{nlab:action, grimaldiDiscreteCombinatorialMathematics2006}
...
\end{equation*}
restricts the larger set to the smaller one such that  $\cgamma{\openset_2}{\dtotal\restriction\openset_2} \subseteq \cgamma{\openset_1}{\dtotal\restriction\openset_1}$. 
\begin{figure}[h]
  \includegraphics[width=\columnwidth]{presheaf.png}
  \caption{\note{amend this so if possible the restriction loses a function}
  \label{fig:atct:presheaf}}
\end{figure}
For example, in \autoref{fig:atct:presheaf}, the set of sections over the interval $\openset_2 = \left[0, 2\pi\right]$ the set of sections includes $sin, cos, constant_0 \subset \cgamma{\openset_2}{\dtotal\restriction\openset_2}$ function. One subset of this interval is $\openset_1 = \left[\frac{\pi}{2}, \frac{3\pi}{2}\right]$, and over this interval is a set of corresponding sections $\cgamma{\openset_1}{\dtotal\restriction\openset_1}$. The constraint that the sheaf functor enforces is that given the sheaves $\sheafc_{\openset_1}, \sheafc_{\openset_2}$, and an inclusion between the opensets, there must be a corresponding restriction between the sections such the set of sections over $\openset_1$ can only contain section functions that were over $\openset_2$. This expectation means that when data containers implement subsetting, which is a combination of inclusion and restriction, no new data is introduced. 

While presheaves preserve the rules for sets of sections, sheaves add on conditions for verifying that individual sections over subspaces are part of a cohesive section over the whole space. 
...
\end{equation}
between a graphic subspace $\opensetg \subseteq \gbase$ and data subspace $\openset \subseteq \dbase$. The functor $\vindex$ is surjective such that for every point $\dbasepoint \in \openset$ there is a corresponding set of points $\{\gbasepoint | \gbasepoint \in  \vindexpre(k)\}$ for all $\gbasepoint \in \opensetg$. We construct the map as going from graphic to data because that encodes the notion that every visual element traces back to the data in some way. We define $\vindex$ as a surjective map because it allows us to express that a union of graphic spaces $S_i$ maps to single data point $\dbasepoint$, which allows us to express visual representations of a single record that are the union of many primatives, discussed in \autoref{eq:artist:operator}, such as multipart glyphs (e.g boxplots) and combinations of plot types (e.g line with point markers). 
...
Since we have defined a function \vindex\ between two spaces $\dbase, \gbase$, we can construct functors that transport sheaves over each space to the other\cite{harder2008lectures}. This allows us to describe what data we expect at each graphic index location and what graphic is expected at each data index location. Transport functors compose the indexing map \vindex\ with the sheave map to say that a record \dsection\ at \dbasepoint\ is at all corrresponding \gbasepoint\ and that a function \gsection\ over one point \gbasepoint\ is the same function at all points $\gbasepoint \in \gbase$ that correspond to the same record index \dbasepoint.
