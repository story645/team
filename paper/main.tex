...
We define a functor
\begin{equation}
  \vindexc: \opensetgc \rightarrow \openset 
\end{equation}
that expresses the constraint that every region in a visualization $\opensetg \subseteq \gbase$ must map to a corresponding set $\openset \subset \dbase$. We define the functor $\vindexc$ such that the data base space \dbasec\ is a deformation retraction\cite{nlab:deformation_retract} of the graphic space $\gbasec$. To simplify matters, in this paper, we construct the graphic space as a constant multiple of the base space such that 
\begin{equation}
  \vindexc: \underbrace{\opensetc\times[0,1]^{n}}_{\opensetgc} \rightarrow \opensetc 
\end{equation}
where $ n = dim(\dbase) - dim(\gbase)$ if $dim(\dbase)<dim(\gbase)$ and otherwise $ n = dim(\dbase)$. 
\begin{equation}
