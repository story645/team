...
\begin{equation}
  ...
  \begin{tikzcd}
    \textcolor{source}{c} \arrow[rr, "F", color=functor, alpha=.5] \arrow[rrrr, "G", bend left, color=functor, alpha=.5] \arrow[dd, "f"', color=source] &  & \textcolor{target}{F(c)} \arrow[dd, "F(f)"', color=target] \arrow[rr, "\alpha_{c}", color=artist] &  & \textcolor{target}{G(c)}\arrow[dd, "G(f)", color=target] \\
                                                                    &  & {} \arrow[rr, "\alpha" description, dashed, color=artist]       &  & {}                      \\
    \textcolor{source}{c^{\prime}} \arrow[rr, "F", color=functor, alpha=.5] \arrow[rrrr, "G", bend right, color=functor, alpha=.5]        &  & \textcolor{target}{F(c^{\prime})} \arrow[rr, "\alpha_{c^{\prime}}", color=artist]   &  & \textcolor{target}{G(c^{\prime})}          
    \end{tikzcd}
\end{equation}
...
\begin{equation}
  ...
  \begin{tikzcd}
    {\Gamma(\opensetc_1, \dtotalc)} \arrow[dd, "\vartistc_{\opensetc_1}"'] &                                                                                                                                   &  &                                                                                                         & {\Gamma(\opensetc_2,\dtotal)} \arrow[dd, "\vartistc_{\opensetc_2}"] \arrow[llll, "\iota^*"', hook'] \\
                                                                           & \opensetc_1 \arrow[lu, "\sheafc_{\vindex^*}" description] \arrow[ld, "\sheafc_{\vartistc}" description] \arrow[rr, "\iota", hook] &  & \opensetc_2 \arrow[ru, "\sheafc_{\vindex^*}" description] \arrow[rd, "\sheafc_{\vartistc}" description] & \\
    {\Gamma(\opensetc_1,\gtotalc)} & &  && {\Gamma(\opensetc_2,\gtotalc)} \arrow[llll, "\iota^*", hook']                                       
    \end{tikzcd}
\end{equation}

In \autoref{eq:artist:morphism-of-sheaf-data}, $\dfuncc$ was introduced as a morphism between sheaf functors $\sheafc^{\vindex^*}$. Since the artist is a functor $\vartist: \sheaf^{\vindex^*} \rightarrow \sheaf_{\vartist}$, it follows from \autoref{eq:related-work:equivariance:functor_diagram} that 
\begin{equation}
  \begin{tikzcd}
    \sheafc_{\vindex^{*}} \arrow[d, "\dfuncc"'] \arrow[r, "\vartistc"] & \sheafc_{\vartist} \arrow[d, "\dfuncc^{\prime} \coloneqq\vartist(\dfuncc)"] \\
    \sheafc_{\vindex^{*}} \arrow[r, "\vartistc"']                      & \sheafc_{\vartist}                                                         
    \end{tikzcd}
\end{equation}
an arbitrary morphism $\dfunc$ on the data sheaf has a compatible morphism $\dfunc^{\prime}$ on the graphic sheaf. 
