...
\begin{minipage}[c]{0.60\columnwidth}
  \begin{tikzpicture}[commutative diagrams/every diagram]
    \draw node[anchor=north west, minimum width=30pt, minimum height=10pt] (kabc) {};
\begin{axis}[simple axis, at={(kabc)}]
  %\addplot [thick, smooth] {sin(2*pi*x)};
\end{axis}

\draw node[anchor=north east, right=55pt of kabc, minimum width=30pt, minimum height=10pt] (kb) {};
%\begin{axis}[simple axis,at={(kb)}]
%  %\addplot [thick, smooth] {sin(2*pi*x)};
%\end{axis}
\draw[at=(kb)][draw=black](0,0) -- (360,0);
\draw node[anchor=south west, below=of kabc, minimum width=30pt, minimum height=10pt] (ka) {};
\begin{axis}[simple axis,at={(ka)}]
  %\addplot [thick, smooth] {sin(2*pi*x)};
\end{axis}

\draw node[anchor=south east, below=of kb, minimum width=30pt, minimum height=10pt] (kc) {};
\begin{axis}[simple axis, at={(kc)}]
  %\addplot [thick, smooth] {sin(2*pi*x)};
\end{axis}
...
\begin{tikzcd}
  \label{eq:atct:fiber_product}
  \dfiberc^{a}\times_{\dfiber^{c}} \dfiberc^{b} \arrow[d, "\pi_a"] \arrow[r, "\pi_b"] 
  & \dfiberc^{b} \arrow[d, "\pi_c"] 
  & {(temperature, time) \times_{time} (pressure, time)} \arrow[d, "\pi_a"] \arrow[r, "\pi_b"] 
  & {(pressure,time)} \arrow[d, "\pi_c"] \\
  \dfiberc^{a} \arrow[r, "\pi_c"] 
  & \dfiberc^{c}
  & {(temperature,time)} \arrow[r, "\pi_c"]
  & time
  \end{tikzcd}
\end{minipage}\hfill
\begin{minipage}[c]{.65\columnwidth}
