...
\end{figure}
Transposition, as illustrated in \autoref{fig:atct:phi}, is one example of a transformation on the base space $(\dfunch, \dfuncpull)$; transposition is a continuity preserving data transformation that moves every data value to a new location such that the values are unchanged and connected in the same way. Scaling, as shown in \autoref{fig:atct:phi} is a transform $\dfunct$ that changes each data value, in this example by multiplying each value by two. As defined in \autoref{eq:atct:morphism:all}, these transformations can combine to yield a matrix that is both transposed and scaled. The matrix in \autoref{fig:atct:phi} is visualized as a heatmap \autoref{fig:related-work:equivariance} and, as discussed in \autoref{sec:related-work:equivariance}, the heatmap is illustrating equivariance. In the $(\dfunch, \dfuncpull)$ case, the data to color map has not changed, but the column bands of color have become row bands. In the $\dfunct$ case, the data has been scaled by two and the color encoding has been scaled by two, but the bands remain column ordered. When the data is transposed and scaled, then the heatmap is also transposed and the colors are scaled.   
