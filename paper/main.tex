...
\begin{definition} A \textbf{record} is a function $\delement: \fnames \rightarrow \mathscr{U}_{\sigma}$ and the set of records on $\pi_{\sigma}$ is denoted $\Gamma^{\pi}(\sigma)$.
\end{definition}
Records must return an object of type $\sigma(\fname) \in \textbf{DT}$ for each field $c \in C$. Spivak then constructs tables as sections $\dsection: \dbase \rightarrow \Gamma^{\pi}(\sigma)$ from a key space $\dspace$ to the set of records $\Gamma^{\pi}(\sigma)$ on the schema bundle. Building on Spivak's construction, we define the fiber space in this work as the set of records 
\begin{equation*}
...
\end{equation*}
such that the preimage of a point is the corresponding domain $\pi^{-1}(\dbasepoint) = \dfiber_{k} = \mathscr{U}_{{\sigma}_{\dbasepoint}}$. This construction of the fiber as a schema bundle allows for the unique representation of data fields that have identical domains because they have the same underlying data type while still retaining the richness of a formal typing system. 
...
\subsubsection{intrepretation/example}
\begin{figure}[h!]
...
\note{add definition of functor + presheaf}
...
\note{make these formal definitions again}
