...
\begin{figure}[h!]
  ...
  \caption{The \texttt{line} and \texttt{image} visual algorithms take in the same array of data. The \texttt{line} assumes that the array is a list of lines, while the \texttt{image} assumes that the array is an image.  \label{fig:related-work:continuity:borked}}
\end{figure}
...
\label{sec:atct:io}
We define categorical types for the topology and the fibers because categories are defined both by the objects of the category and the valid morphisms on the object. This allows for the formal construction of an equivariance aware type that holds the topology, field, type, and what changes to either must be preserved. 
...
The conditions on sheaves express the presheaf properties of continuity preservation as applied to sections, which as mentioned in \autoref{sec:atct:fiber-bundles} are how data is modeled in this framework. A sheaf is a presheaf that satisfies the locality and gluing axioms \cite{bakerMathsSheaf}. 
\begin{definition}
