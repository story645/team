...
\begin{definition}
  An \textcolor{action}{\textbf{action}} \textit{act} of $G$ on $X$ is a function  $act: G \times X \rightarrow X$. An action has the properties of identity $act(e, x) = x$ for all $ x \in X$ and associativity $act(g, act(f, x)) = act(f \circ g, x)$ for $f,g \in G$ \cite{nlab:action, wki}.
\end{definition}
...
\begin{table}[h!]
  ...
  \begin{tabular}{|r | c c c|}
    ...
    &\textcolor{base}{point}/\textcolor{base}{openset}/\textcolor{base}{base space} & \textcolor{fiber}{fiber space} & \textcolor{total}{total space}\\
     &  location/subset/topology & fields &  dataset type\\
    \hline
   ...
   Graphic & $\gbasepoint \in \opensetgc \subseteq \gbasec$ & \gfiberc & \gtotalc\\
   \hline
  \end{tabular}
