...
\begin{equation*}
  ...
  \begin{description}
      ...
      \item[actual]{$\extractmc:\gsectionc \mapsto (\opensetgc \xrightarrow{\extractmc_{\gsectionc}} \measurec)$}
...
\subsection*{Compositing \vchannelc's}
...
\subsection*{Sharing \vchannelc's}
\begin{equation}
...
\subsubsection{Shared Visual Encodings}
\begin{equation}
  \begin{tikzcd}[row sep=2.5em, column sep=.75sem]
    &  & \imartistsub{ab}{\opensetgc}{\gtotalc\restriction\opensetgc}
    \arrow[rrd, "\extractmc_{bc}", color=monoid] &  &  \\
\cgamma{\opensetc}{\vtotalc^{a}\restriction_{\opensetc}\times \vtotalc^{b}\restriction_{\opensetc}} 
\arrow[rru, "\vmarkc_{ab}", color=artist] 
\arrow[d, "\pi_a"', color=total] &  &  &  & 
{{\textcolor{set}{Hom}(\opensetgc, \measurec^{ab})}} 
\arrow[d, "\measurec^{ab}\restriction_a", color=set]  \\
\cgamma{\opensetc}{\vtotalc^{a}\restriction_{\opensetc}} 
\arrow[rr, "\simeq", dotted]  &  & 
{{\textcolor{set}{Hom}(\opensetc, \measurec^{a})}} 
\arrow[rr, "\vindexpullc", color=functor] &  & 
{{\textcolor{set}{Hom}(\opensetgc, \measurec^{a})}} \\
\cgamma{\opensetc}{\vtotalc^{a}\restriction_{\opensetc}\times \vtotalc^{c}\restriction_{\opensetc}} 
\arrow[rrd, "\vmarkc_{bc}", color=artist] 
\arrow[u, "\pi_a", color=total]  &  &  &  & 
{{\textcolor{set}{Hom}(\opensetgc, \measurec^{ac})}} 
\arrow[u, "\measurec^{ab}\restriction_a"', color=set] \\
 &  & \imartistsub{ac}{\opensetgc}{\gtotalc\restriction\opensetgc}
 \arrow[rru, "\extractmc_{ab}", color=monoid] &  & 
\end{tikzcd}
\end{equation}
