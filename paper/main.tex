...
\subsubsection{Connectivity: Base Space \dbase}
The \textcolor{base}{base space} is a topological space \dbasec\ with points $\dbasepoint \in \dbase$ and topology $\mathcal{T}_{\dbase}$. For our purposes, $\dbase$ is a set of points that encodes the underlying connectivity of the dataset. For example, $\dbase$ could be the discrete points, interval, or plane in \autoref{fig:related-work:continuity:ktypes}. On this set \dbase, we can define subsets of connected elements, called opensets \openset. An openset \openset\ is a generalization of an open interval, meaning an openset contains points inside a bounded region on \dbase and excludes the points on the boundary. A topology $\mathcal{T}_{\dbase}$ defined on \dbase\ is a collection of opensets such that the empty set and \dbase are in $\mathcal{T}_{\dbase}$,and every union and intersection of opensets in $\mathcal{T}_{\dbase}$ are also in $\mathcal{T}_{\dbase}$ \cite{TopologyVsTopology,bradleyTopologyCategoricalApproach2020}. The topology is encoding the notion that the connectivity is expected to be preserved regardless of how small an openset or large a union of opensets.  

\subsection{Field Types: Fiber Space \dfiber}
The \textcolor{fiber}{fiber space} is a topological space \dfiberc\ that is the preimage of the projection function $\pi$ at a point in the base space $\dfiber_{\dbasepoint} = \pi^{-1}(\dbasepoint)$, meaning that all points in a fiber over a space $\dfiber_{\dbasepoint}$ map to the same point $\dbasepoint$. The fiber space can be used to encode the space of all possible values of a data field and the fibers can be named, as described by Spivak \cite{spivakDatabasesAreCategories2010,spivakSIMPLICIALDATABASES}. The space of all possible values is analogous to a CS data type; therefore the fiber space can be used to encode the data types of the data fields. This set theoretic formulation of field types also supports encoding valid operations on the type as part of the type, akin to methods on an object, as part of the field structure. For example, the Steven's measurement types are defined by the mathematical group structure on the measurment fields \cite{stevensTheoryScalesMeasurement1946,leaFormalizationMeasurementScale, thomasMathematizationNotMeasurement2014}, and often serve as the basis of the equivariance described in \autoref{sec:related-work:equivariance}.
...
\end{equation}
Each section function takes as input a point in the basespace  and returns a single point in the fiber  $\dsection: \dbasepoint \rightarrow \dfiber_{\dbasepoint}$. In a trivial bundle $\dtotalc = \dbasec \times \dfiberc$ such that all fibers are equal $\dfiberc \equiv \dfiberc_{\dbasepointc}\;\forall \dbasepointc \in \dbase$; therefore we can define a global section $\dsection \in \Gamma(\dbase, \dfiber)$. In non-trivial (general) bundles, fibers are isomorphic $\dfiberc \simeq \dfiberc_{\dbasepointc}\;\forall \dbasepointc \in \dbase$ but not necessarily equal, which means that we usually cannot define a global section. Instead, all fiber bundles are locally trivializable, $\dtotal\restriction_{\openset_{k}} = \openset_{k}\times \dfiber$ for all open neighborhoods $\dbasepoint \openset \dbase$ for all points $\dbasepoint \in \dbase$. This allows us to define sections locally, meaning $\dsection\restriction_{\openset} \in \Gamma(\openset_{\dbasepoint}, \dtotal\restriction_{\openset_{\dbasepoint}})$. All data can be expressed as a collection of local sections, which are maps from the indexing space \dbase\ to the field space \dbase, and data that is embedded in trivial bundles can be expressed as a single section. 
 

\begin{figure}[h!]
  \centering
  %needs cropping
  \includegraphics[trim={2cm 2cm 2cm 2cm}, clip]{fbtau.png}
...
\note{maybe unpack each piece of this equation into a seperate equation/walk through, from U to fiber when trivial}  
\begin{equation}
...
Equivariance in a visual context was formally defined as the preservation of a binary operator from one domain to another by Mackinlay\cite{mackinlayAutomaticDesignGraphical1987}. We generalize binary operations to actions $\dfunc \in \dfuncset$ on elements of the bundle $\delement \in \dtotal$. This is expressed with a composition function  $act: \dfuncset \times E \tightarrow \dtotal$ that applies the action to the element such that $\dfunc(\delement_0) = \delement_1, \delement_0, \delement_1 \in \dfunc$. We generalize to a family of actions because that allows for expanding the set of allowable transformations on the data beyond a single operator. 
...
\end{equation}
\note{\ci_*rho(k)(s)=rho(s) for s in \xi^{-1}(k)
