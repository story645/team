...
The graphic category $\mathcal{\gtotal}$ consists of the functor $\sheaf: \mathcal{S}^{op} \rightarrow \Gamma(\mathcal{S}, \gtotal)$, which we also denote as $\sheaf(\gtotal)$. The category $\mathcal{S}$ is the category of open sets over the graphic base space \gbase\, and $\Gamma(\mathcal{S}, \gtotal)$ is the set of sections $\gsection$ over $\mathcal{\gbase}$. The artist is a morphism of sheafs $\vartist: \mathcal{\dtotal} \rightarrow \mathcal{\gtotal}$; therefore it is a map between functors, which means that the artist is a natural transformation\cite{SheafMathematics2021,bradleyWhatNaturalTransformation}.
...
\begin{equation}
  ...
  \begin{tikzcd}
    \mathcal{\dtotal} \arrow[r, "\vartist"] \arrow[d, "{Hom_{\mathcal{\dfiber}}(\dfiber, \dfiber)}"'] & \mathcal{\gtotal} \arrow[d, "\vartist_{Hom_{\mathcal{\dfiber}}}"] \\
