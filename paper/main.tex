...
\label{sec:atct:equivariance:category}
...
\subsection{Sheaves}
\label{sec:atct:continuity:sheaves}
...
\begin{equation}
  ...
  \mathcal{\dtotalc} \textcolor{artist}{\xrightarrow{\vartist}} \mathcal{\gtotalc}
\end{equation}
...
\begin{definition} \label{def:category:K} The category of open sets $\mathcal{\dbasec}$ consists of
  \begin{itemize}
    \item \textit{objects} open sets $\opensetc_{i} \in \{\opensetc\}$ in the topological space $(\dbasec, \mathscr{T})$), including the empty set $\varnothing$ and the maximum set \dbase. 
    \item \textit{morphisms} $\iota: \opensetc_{i} \hookrightarrow \openset_{j}$ for all  $\opensetc_i, \opensetc_j \subset \dtotalc$
  \end{itemize} 
\end{definition}
...
\begin{definition} The category of open sets $\mathcal{\gbasec}$ in the topological space $(\gbasec, \mathscr{T})$ consists of open set \textit{objects} $\opensetgc \in \{\opensetgc\}$, including the empty set $\varnothing$ and the maximum set $\gtotalc$ and \textit{morphisms} $\iota: \opensetgc_i \rightarrow \opensetgc_j$.  
\end{definition}

The data space \dbase\ is defined via the surjective function $\vindex$ such that regions in \gbase\ collapse to points in \dbase\
\begin{equation}
  \label{eq:artist:vindex}
  \vindexc: \gtotalc \textcolor{artist}{\rightarrow} \dtotalc
\end{equation}
Since \dbase\ is defined as a deformation retract of the graphic topology \gbase, the continuity of the points in data space and graphic space are equivalent \cite{nlab:deformation_retract}. One method of constructing the graphic space is by expanding the dimensionality of the data \dbase\ to match the spatial dimensionality of the rendering by multiplying the data space by  intervals $I$. 
...
\subsubsection{Data}
As mentioned in \autoref{sec:atct:sheaves} sheaves are a powerful tool for keeping track of data that lies over topological spaces; therefore, we model the type of data an artist expects as a sheaf on a fiber bundle with total space \dtotalc and basespace \dbase.  
...
We model graphics as a sheaf from an open set in the graphic base space to sets of sections of graphic generating functions
\begin{equation}
  \label{eq:artist:graphic_sheaf_gamma}
  \sheafc_{\gbasec}: \opensetgc \textcolor{sheaf}{\rightarrow} \cgamma{\opensetgc}{\gtotalc}
\end{equation}
and we define the set of sections such that 
\begin{equation}
...
Given the data sheaf $\sheaf_{\dbase}$, graphic sheaf $sheaf_{gbase}$, functor $\vindex$ between $\gbase$ and $\dbase$, we can define the artist as a morphism between sections of sheaves
\begin{equation}
  \begin{tikzcd}[row sep=huge]
    \{\gsectionc: \opensetgc \textcolor{section}{\rightarrow} \gtotalc\restriction_{\opensetgc}\} \arrow[rr, "\vindexpushc", color=functor]                                                     &  & \{\gsectionc: \vindexprec(\opensetc) \textcolor{section}{\rightarrow} \gtotalc\restriction_{\vindexprec(\opensetc)}\}\\&  &\\
    \{\dsectionc: \vindexc(\opensetgc) \textcolor{section}{\rightarrow} \dtotalc\restriction_{\vindexc(\opensetgc)}\} \arrow[uu, "\vartistc_{\vindexpullc}: \vindexpullc \dsectionc \textcolor{artist}{\rightarrow} \gsectionc", color=homset] \arrow[rr, "\vindexpullc", color=functor] &  & \{\dsectionc: \opensetc \textcolor{section}{\rightarrow} \dtotalc\restriction_{\opensetc}\} \arrow[uu, "\vartistc_{\vindexpushc}: \dsectionc \textcolor{artist}{\rightarrow} \vindexpushc \gsectionc"', color=homset] \arrow[lluu, "\vartistc" description, dashed, color=artist]
    \end{tikzcd}
\end{equation}
...
\note{We can define a monoid on the set of artists w/ + such that artists are composoble. doing so lets us}
...
\subsubsection{Pulled Back Data}

We model the type of data we have as fiberbundles, building on the work of Butler et al. \cite{butlerVisualizationModelBased1989,butlerVectorBundleClassesForm1992}. Specifically we denote our data as the bundle \dtotal\ over the base space \dbase. We pull this bundle over the graphic space \gbase\, establishing an association between regions in the graphic and corresponding data. 

Given the map \vindex\ (\autoref{eq:artist:vindex}) between data and graphic basespaces, we can introduce a pullback function that copies sections of the data bundle over the data base space to the graphic base space. 
. 
\begin{equation}
  \label{eq:artist:vindex}
  \begin{tikzcd}
    \dtotalpullc \arrow[rr, "\vindexpullc", color=artist]                                                   &  & \dtotalc                          \\
                                                                                                                                    &  &                                   \\
    \gbasec \arrow[rr, "\vindexc", color=artist] \arrow[uu, "\dsectionpullc", dashed, color=section] \arrow[rruu, "\vindexc\circ\dsectionc" description, dashed, color=section] &  & \dbasec \arrow[uu, "\dsectionc"', color=section]
    \end{tikzcd}
\end{equation}
