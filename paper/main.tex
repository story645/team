...
The category $\mathcal{\dfiber}$ is a subcategory of an arbitrary category $\mathcal{C}$. $\mathcal{\dfiber}$ is, meaning it has one object \texttt{\dfiber} and a functor\footnote{The functor $\otimes$ is a bifunctor because it takes as input the product of two categories \cite{nlab:bifunctor}} $\otimes:\mathcal{\dfiber}\times{\mathcal{\dfiber}}\rightarrow \mathcal{\dfiber}$\cite{barrCategoryTheoryComputing}.

\begin{definition} 
  \label{def:category:F} 
  The category $\mathcal{\dfiber}$ consists of one object $\dfiber$ with maps back to itself $\monoid: \dfiber \rightarrow \dfiber$. 
\end{definition}


%F is the one object category with maps back to itself M: F\rightarrow F, fiber and all its allowed maps. morphsims in categroy C that go back to same object fixed F, we note that this is also a category w/ one object, a one object categry is a monoid

The object $\texttt{\dfiber}$ is an object of some category $\mathcal{C}$, which allows $\texttt{\dfiber}$ to encode most data fields. For example, a lists of strings is an instance of an object in \textbf{Set}, networks are an instance of \textbf{Graph}, and images are vector spaces which are a specific type of topological space \textbf{Top}. We note that a one object category is a monoid \cite{nlab:mo}
...
%insteaf of trying to define gamma, write this up as a paragaraph... we use the convention \Gamma(U,E) to denote, the map from U_i to gamma U_e...defines a presheaf which is also a sheaf.... continue w/ global sections 
\begin{definition} For each open set object $U_{i}$ of category $\mathcal{\dbase}$, there is a set $\Gamma(U_i, \dtotal)$. The elements of this set are the sections $\{\tau^{j}:U_i \rightarrow \dfiber_{U_{i}}\}_{j\in I}$ of the fiber bundle \dtotal\ over open set $\U_{i}$. The sections $\Gamma(\dbase, \dtotal)$ are called the global sections of $\dtotal$. \cite{SheafMathematics2021,spanier1989algebraic}.
\end{definition}
...
\begin{definition} The category $\mathcal{\dtotal}$ consists of 
  \begin{itemize}
    ...
    \item\textit{morphisms} allowed maps from sheaf to sheaf, when restricted to a fiber, that should be a map from a fiber to fiber.  
