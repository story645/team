...
\note{roadmap the rest of this section}
...
\begin{definition} The category $\textcolor{section}{\mathcal{T}}$ consists of 
  \label{def:category:Tau}
  \begin{itemize}
  \item \textit{object} the set of sections $\Gamma(U_i)$ where $U_i$ can be any open set object in $\mathcal{\dbase}$
  \item \textit{morphisms} fiber maps $Hom(F, F)$, because sections are maps from a point in the base space to a point in the fiber $\tau: \dbase \rightarrow \dfiber$
  \end{itemize}
\end{definition}
...
\begin{definition} The category $\mathcal{\dtotal}$ consists of 
  \begin{itemize}
    ...
    \item \textit{morphisms} arbitrary endomorphic\cite{fongInvitationAppliedCategory2019} bundle maps $f: \dtotal\rightarrow \dtotal$. When the sheaf is restricted to the stalk\cite{StalkSheaf2019} $\mathscr{F}_{k}$ over a point $\dbasepoint\in \dbase$, then allowable morphisms are $f \in Hom_{\mathcal{\dfiber}}(\dfiber,\dfiber)$. 
  \end{itemize}
\end{definition}

 The artist takes as input the sheaf object $\mathcal{E}\coloneqq\sheaf_{\mathcal{E}}$ because the sheaf is a way of keeping track of how data is glued together \cite{ghristElementaryAppliedTopology2014}. This bookkeeping is done via the inclusion maps that are preserved by the presheaf, as illustrated in \autoref{eq:artist:presheaf}. Sheafs are presheafs where sections are defined over unions of open sets $U \cup_{U\in \mathscr{U}}$ \cite{spanier1989algebraic, nlab:sheaf}. As illustrated in \autoref{eq:artist:presheaf}, the functor $\sheaf_{\mathcal{\dtotal}}$ is a contravariant functor from a category to a category of sets \textbf{Set} \cite{spanier1989algebraic,SheafMathematics2021,nlab:presheaf}, which means it is by definition a presheaf.
...
Arbitrary morphisms on sheafs $f:\mathcal{\dtotal} \rightarrow \mathcal{\dtotal}$ are natural transforms \cite{SheafMathematics2021,bradleyWhatNaturalTransformation}, meaning that the morphisms $f$ compose with the inclusion maps $\iota$ \cite{fongInvitationAppliedCategory2019}. As defined in \autoref{def:category:Tau}, the sets of sections $\Gamma(U_{i},\dtotal)$ are objects of the category $\mathcal{T}$. 
...
The sheaf $\sheaf_{\mathcal{\dtotal}}$ over a limit of open sets that contain a point $\dbasepoint \in \dbase$ is approximately the same as a sheaf over a point \dbasepoint\ and is called the stalk $\sheaf_{\mathcal{\dtotal}}|_{k}\coloneqq \lim\limits_{U\ni k} \Gamma(U, \dtotal)$ \cite{StalkSheaf2019}. The fiber space over a point is embedded in the stalk over that point $\dfiber_{k}\hookrightarrow \mathscr{F}_{k}$. Our criteria for equivariance is that bundle maps $f$ that are in the set of fiber to fiber maps $Hom_{\mathcal{\dfiber}}(\dfiber,\dfiber)$ are preserved. The maps $f\in Hom_{\mathcal{\dfiber}}(\dfiber,\dfiber)$ include the types of data transformations described in \autoref{sec:related-work:equivariance}. When a fiber bundle is trivial, meaning $\dtotal=\dbase \times \dfiber$, then we can dispense with localization and directly consider the fiber maps $Hom(\dfiber_{k}, \dfiber_{k})$ over a point \dbasepoint. In \autoref{sec:artist:construction}, we assume that the fiber bundles are trivial.

The graphic category $\mathcal{\gtotal}$ consists of the functor object $\sheaf_{\mathcal{\gtotal}}: \mathcal{S}^{op} \rightarrow \Gamma(\mathcal{S}, \gtotal)$ and the sheaf morphisms $g:\sheaf_{\mathcal{\gtotal}} \rightarrow \sheaf_{\mathcal{gtotal}}$. The category $\mathcal{S}$ is the category of open sets over the graphic base space \gbase, and the category $\mathcal{P}$ is the set of sections $\gsection$ over $\mathcal{\gbase}$. The category of possible graphics $\mathcal{P}$ has as objects all graphics with a shared continuity and display space; bar plots and scatter plots on a 2D screen are two examples of objects in $\mathcal{P}$. As will be discussed in \autoref{sec:artist:construction}, the  $\vartist$ is constructed in such a way that it generates a single type of graphic. Therefore, we define the object as a morphism from data to the sheaf mage of the artist in the graphic space $\vartist: \mathcal{\dtotal} \rightarrow Im_A \in $
...
& \mathcal{\dbase}^{op} \arrow[uu, "\mathcal{\dtotal}", bend left] 
  \arrow[uu, "\mathcal{\dtotal}"', bend right] 
  & & 
& \mathcal{\gbase}^{op} \arrow[uu, "\mathcal{\gtotal}", bend left] 
  \arrow[uu, "\mathcal{\gtotal}"', bend right] &   
  \end{tikzcd}
\end{equation}

% f is a natural transform, all transforms on the sheafs commute
% g is a natural transform ...
% space of g->g is too big, so we look at Im...
% b/c it's a natural transform, - f carries i over b/c it's a natural transformation 
...
\begin{equation}
  ...
  \vartist:\mathcal{\dtotal}\rightarrow{\mathcal{\gtotal}} 
\end{equation} 
which means that the artist is a natural transformation\cite{SheafMathematics2021,bradleyWhatNaturalTransformation}.

% such that (9)

Extra property that implies equivariance is preserved is illustrated in the diagram which must commute
%% note Im_A in \H, A_M (lifting/pushforward)
\begin{equation}
...
%% such that monoid on this side is monoid on that side

In \autoref{sec:artist:construction}, we show a method of constructing the artist such that the constraints of \autoref{eq:artist:morphism-of-sheaf} and \autoref{eq:artist:equivariance} can be satisfied, thereby ensuring equivariance and the preservation of continuity. We then use this formulation to guide the development of visualization 
library components in \autoref{sec:case-study}.

\subsection{Artist Composition}
\label{sec:artist:union}
\begin{figure}[!h]
\centering
\subfloat[+]{\includegraphics[width=.4\columnwidth]{exploding_artist.png}%
\label{fig:artist:union:glue}}
%\hfil
\subfloat[$\oplus$]{\includegraphics[width=.4\columnwidth]{combined_artist.png}%
...
\end{figure}
%% artist composition is fiber products/sums of artists on sheafs -> closed cartesian category maybe? 
...
\subsubsection{Superposition: union}
