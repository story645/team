...
\end{figure}
Continuity, which Wilkinson terms topological properties \cite{wilkinsonGrammarGraphics2005}, describes how elements in a data set are organized. Wilkenson provides the examples of values that are isolated from each other, and therefore discrete, and values lying on a continuum or in a compact region. We propose that a robust model of continuity, which is how the elements in a dataset are connected to each other, provides a way to develop library components that can 
