...
\begin{tikzcd}
  c \in Ob(\mathcal{C}) \arrow[d, "f"', shift right=8] \arrow[r, "F"] & F(c) \in Ob(\mathcal{D}) \arrow[d, "F(f)", shift right=8] \\
...
\begin{equation}
  ...
  \begin{tikzcd}
    c \arrow[rr, "F"] \arrow[rrrr, "G", bend left] \arrow[dd, "f"'] &  & F(c) \arrow[dd, "F(f)"'] \arrow[rr, "\alpha_{c}"] &  & G(c) \arrow[dd, "G(f)"] \\
                                                                    &  & {} \arrow[rr, "\alpha" description, dashed]       &  & {}                      \\
    c^{\prime} \arrow[rr, "F"] \arrow[rrrr, "G", bend right]        &  & F(c^{\prime}) \arrow[rr, "\alpha_{c^{\prime}}"]   &  & G(c^{\prime})          
    \end{tikzcd}
\end{equation}
...
In \autoref{eq:artist:morphism-of-sheaf-data}, $\dfuncc$ was introduced as a morphism between sheaf functors $\sheafc^{\vindex^*}$. Since the artist is a functor $\vartist: \sheafc^{\vindex^*} \rightarrow \sheaf_{\vartist}$, it follows from \autoref{eq:related-work:equivariance:functor_diagram} that 
\begin{equation}
  \begin{tikzcd}
