...
\begin{definition}
The \textcolor{base}{base space} \dbasec\ is a topological space, which means it is a a set $\dbase$ with a collection of open sets surrounding each element in the set. For each point $\dbasepoint in \dbase$, there is a collection of subsets $\{U\} \subset \dbase$ such that $x \in U \subseteq \dbase$ \cite{spanier1989algebraic, bradleyTopologyCategoricalApproach2020}  
\end{definition}
...
\begin{figure}[!h]
  \centering
  \subfloat[\dbasec_a \sqcup_{\dbasec_c} \dase_b]{\includegraphics[width=1.2in]{union_k.png}%
