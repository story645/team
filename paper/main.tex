...
\begin{definition} The category $\mathcal{\dtotal}$ consists of 
  ...
  \end{itemize}
\end{definition}
% continuous functions on a bundle with a limit around k is the stalk - maps from open neighborhood to point in neighborhood - topologically collapse open neighborhood to to point in neighborhood, pullback map goes one way, pullback other - gives a way to embed the fiber in the sections/constant sections, in the stalk F sites as a constant section, stalk contains F over point, map of O(F1)->O(F), induces map from stalk to stalk, F\hookright arrow stalk F, map F to F, require that map be in Hom(F,F)
% if E = K \times F, can get rid of localization and just consider fiber maps directly 
...
%H is the codomain, ImA is the range, K & S -> 
% clipping problem: shift lines and stuff w/o shifting frame, when we shift the frame, only shifting the bits of H that belongs to the artist  
