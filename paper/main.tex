...
\usepackage{pgfplots} %numberline
...
\IEEEPARstart{V}isualization is a structure preserving mapping from data to graphic. One way of verifying that the structure of the data is present in the graphic is to verify that the data set can be distilled from the graph, i.e. that the values read off a bar chart are equivalent to the values used to construct the bar chart. Since structure preservation requires that the $data\leftrightarrow graphic$ be bidirectional, changes to either data or graphic are expected to propogate to the other side of the transformation\cite{kindlmannAlgebraicProcessVisualization2014}. In this paper, we propose a methodology for describing structure, verifying structure preservation, and specifying the conditions for constructing a structure preserving map between data and graphics. We use category theory, which is the math of structures and their relations, to explicitly specify what it means for a visualization mapping to preserve structure in an explicit, comprehensive, and consistent way. We encode the structure of the data and graphics in a uniform generalizable manner by using a mathematical structure from algebraic topology called a fiber bundle; doing so allows us to discuss everything from lines to complex ND spaces with the same interface. 

We use category theory and algebraic structures because they can be directly translated into software design specifications and our goal is to improve visualization tools. The algebraic structures can be translated into analogous programmatic types, while category theory facilitates developing a functional design framework that describes function composition in terms of functional algebraic operations. The typing system and composition inherent in a functional design encourage library developers to build complex visualization components from simple verifiable parts that have few side effects\cite{huHowFunctionalProgramming2015, hughesWhyFunctionalProgramming1989}. These categorically specified components could be built as a standalone library and integrated into existing libraries because they are inherently self-contained. Furthermore we see the application of these ideas in low level visualization library design as our motivation, and we hope these ideas will help shape the reorganization of critical data visualization libraries, such as Matplotlib. The contribution of this paper is a mathematical framework for describing structure preserving visualization components in a manner that is generalizable to different types of inherent structure. This framework also provides guidance for the construction and testing of structure preserving visualization library components.

\section{Related Work}
We build on extensive work codifying the structure of visualization data and corresponding graphics and use the concepts of \textit{homomorphism} and \textit{equivariance} to express how that structure should be preserved. Structure is considered preserved when the input and output change in tandem, and these symmetries are often modeled as \textit{group actions}, which are a way for a group\footnote{A \textit{group} is a set with an associative binary operator, an identity, and an inverse operation} to transform some other set:

\begin{definition}\label{def:related-work:action}\cite{nlab:action, grimaldiDiscreteCombinatorialMathematics2006}
  An \textcolor{action}{\textbf{action}} \footnote{Throughout this paper, we augment the math with color to group conceptually similar objects and functions\cite{headMathAugmentationHow2022}, for example \textcolor{action}{actions} and \textcolor{action}{sets of actions}. This color coding carries through to the figures.} of \textcolor{action}{$G = (G,\circ, e)$} on $X$ is a function  $act: \textcolor{action}{G} \times X \rightarrow X$. An action has the properties of identity $act(\textcolor{action}{e}, x) = x$ for all  $x \in X$ and associativity $act(\textcolor{action}{g}, act(\textcolor{action}{f}, x)) = act(\textcolor{action}{f} \circ \textcolor{action}{g}, x)$ for $\textcolor{action}{f},\textcolor{action}{g} \in \textcolor{action}{G}$.
\end{definition}

Given a group $G$ that acts on both the input $X$ and the output $Y$ of a function $f: X \rightarrow Y$

 \begin{definition}
$f$ is \textbf{equivariant} when $f(act(g,x)) = act(g,f(x))$ for all $g$ in $G$ and for all $x$ in $X$\cite{nlab:equivariant, pittsNominalSetsNames2013}.
 \end{definition}
In an equivariant transform, mapping the data after acting on it $f(act)$ is equivalent to acting on the data after mapping it $act(f)$ since $G$ acts on both. A homomorphism does not require that the input and output are of the same type; instead 
...
The connectivity and relative positioning of elements in a dataset is broadly its topology\cite{wilkinsonGrammarGraphics2005} and the expectation that it be be reflected in the visual representation is present in even the earliest maps \cite{friendlyBriefHistoryData2008} and in Bertin's assertion that the dimensionality of the data match the class of representation \cite{bertinSemiologyGraphicsDiagrams2011}. In this work, we propose that using the formalism of topology provides us a uniform way to express dimensionality such that we can generalize the expectation that the data and visual dimensionalty match past points, lines, and areas. 
...
A topology is a partitioning of a space into its subspaces in a way where those subspaces can be composed back into the space and the choice of partitions is how we encode the connectivity of the data that we are interested in. For example, the GHCN\cite{lawrimoreGlobalHistoricalClimatology2011} dataset in \autoref{fig:related-work:continuity:ktypes} is a complex spatio-temporal record of weather station data. One way of encoding that each record is independent is to encode the underlying topology as a set of 0D $\cdots$ points, one for each record. Each of these discrete points is also represented visually as a discrete marker in the scatter plot. We can view the timeseries at each station as a 1D $---$ continuous line since weather station data is sampled from natural phenomena. We encode this topological structure as intervals, .i.e [start date, end date], and sets of sub-intervals, and we visually represent this strucure as continuous line plots. A third way of viewing the data is as a sparse sampling of a 2D $\blacksquare$ geospatial grid, where the connectivity is modeled as a surface and sub regions of that surface. In the visualization, that connectivity is expressed as plotting the stations in the map, visually denoting that these are points in the continuous geographical space that is New York State. By using topology as the underlying encoding scheme, we are able to represent the structure of points, lines, surfaces, and more using the same abstractions.


Since topological continuity is generalizable, we propose that encoding topology as part of a dataset type and a visualization type allows us to explicitly check which types of structure a visualization algorithm can preserve. Visual algorithms assume the topology of their input data, as described in taxonomies of visualization algorithms Chi\cite{chiTaxonomyVisualizationTechniques2000} and by Troy and M\"{o}ller \cite{toryRethinkingVisualizationHighlevel2004}, but generally do not verify that input structure. For example, a \texttt{line} algorithm often does not have a way to query whether a list of (x,y) coordinates is the distinct rows, the time series, or the list of stations in \autoref{fig:related-work:continuity:ktypes}. While plotting the time series as a continuous line would be correct, it would be incorrect for a visualization to indicate that the distinct rows or stations are connected in a 1D continuous manner. Encoding the expected input structure would enable a visualization algorithm to reject data that does not explicitely state that it matches those assumptions. This structure can be encoded in a generalizable way by building on Butler's work on using a mathematical structure called fiber bundles as an abstract data representation in visualization \cite{butlerVectorBundleClassesForm1992, butlerVisualizationModelBased1989} where he discusses how bundles can be used to encode data in a consistent manner for the topological and field structures that are common in visualization. We sketch out fiber bundles in \autoref{sec:atct:fiber-bundles}, but his work provides a thorough introduction to bundles for visualization practitioners. 
...
For example, the disjoint union of two bundles, as shown in \autoref{fig:atct:local_trivalizations}, is the coproduct $\dbase^{a} \subseteq_{\dbase^{c}} \dbase^{b}$ over an overlap $\dbase^{c}$ and therefore the inclusion morphism must be commmutative:
\begin{minipage}[c]{0.25\columnwidth} 
  \begin{equation}
    \label{eq:atct:base_union}
  \begin{tikzcd}
    \dbasec^{a}\sqcup_{\dbasec^{c}}\dbasec^{b} 
    & \dbasec_{b} \arrow[l, "\iota_b"', color=base] \\
    \dbasec_a \arrow[u, "\iota_a", color=base] 
    & \dbasec_c 
    \arrow[u, "\iota_c"', color=base] 
    \arrow[l, "\iota_c"', color=base] 
  \end{tikzcd}
\end{equation}
\end{minipage}\hfil
\begin{minipage}[c]{0.65\columnwidth}
  \note{will be replaced by # lines showing intervals}
  \begin{tikzpicture}[commutative diagrams/every diagram]
    ...
    (m-2-2) edge[color=base] (m-1-2)
            edge[color=base] (m-2-1)
    (m-2-1) edge[color=base] (m-1-1)
    (m-1-2) edge[color=base] (m-1-1);
  \end{tikzpicture}
\end{minipage}
...
\begin{tikzcd}
  \dfiberc^{a}\times_{\dfiber^{c}} \dfiberc^{b} \arrow[d, "\pi_a"] \arrow[r, "\pi_b"] & \dfiberc^{b} \arrow[d, "\pi_c"] & {(temperature, time) \times_{time} (pressure, time)} \arrow[d, "\pi_a"] \arrow[r, "\pi_b"] & {(pressure,time)} \arrow[d, "\pi_c"] \\
  \dfiberc^{a} \arrow[r, "\pi_c"]                                                     & \dfiberc^{c}                    & {(temperature,time)} \arrow[r, "\pi_c"]                                                    & time                                
  \end{tikzcd}
\end{minipage}\hfill
\begin{minipage}[c]{.65\columwidth}
  \note{will be little pictures like in 7 sketchs  }
\end{minipage}

This means that when multifield data is decomposed into its component fields, shared records stay the same. For example, the red (temperature, time, pressure) record seperates into (tempereture, time) and (pressure, time) records that share the same red time. Furthermore this time is the same whether it is obtained from the (temperature, time) or (pressure, time) record. This simple test that fields are joined together correctly for the same record is what allows us to reliably combine multiple datasets together on shared properties-for example growing the weather station data from a temporal to spatial dataset by adding the weather at each  location at each time. 
...
\begin{tikzcd}
  ...
  \end{array}\right\}} \\
  \textcolor{base}{\openset_2 = \left(0,2\pi\right)} 
...
\end{equation}
that preserve the categorical structure (operators and morphisms) of the fiber and the contininuity of the data section. As mentioned in \autoref{sec:construction:vtotal}, the total visual space is restricted to the space of data types internal to the library $\vfiber \subset \measure$ and sections are subsets of homsets $\Gamma(\dbase, \vtotal) \subset Hom(\dbase, \measure)$ because sections must be continuous.

The encodeding functions $\vchannel$ are fiberwise transforms such that $\pi(\dtotal) = \pi(\vchannel(\dtotal))$. A consequence of this property is that $\vchannelc$ can be constructed as a pointwise transformation such that  
\begin{equation}
...
\pagebreak
\section{Internel Library Specification}
