...
\begin{equation}
  ...
  \begin{tikzcd}
    \textcolor{source}{c} \arrow[rr, "F", color=functor, alpha=.5] \arrow[rrrr, "G", bend left, color=functor, alpha=.5] \arrow[dd, "f"', color=source] &  & \textcolor{target}{F(c)} \arrow[dd, "F(f)"', color=target] \arrow[rr, "\alpha_{c}", color=nattran] &  & \textcolor{target}{G(c)}\arrow[dd, "G(f)", color=target] \\
                                                                    &  & {} \arrow[rr, "\alpha" description, dashed, color=nattran]       &  & {}                      \\
    \textcolor{source}{c^{\prime}} \arrow[rr, "F", color=functor, alpha=.5] \arrow[rrrr, "G", bend right, color=functor, alpha=.5]        &  & \textcolor{target}{F(c^{\prime})} \arrow[rr, "\alpha_{c^{\prime}}", color=nattran]   &  & \textcolor{target}{G(c^{\prime})}          
    \end{tikzcd}
\end{equation}
...
\begin{equation}
  ...
  \begin{tikzcd}
    ...
    \dbasec \arrow[u, "\dsectionc", color=section] & \gbasec \arrow[l, "\vindexc", color=artist] \arrow[lu, "\dsectionc \circ \vindexc" description] \arrow[u, "\vindexc^*\dsectionc"', dashed, color=section]
    \end{tikzcd}
\end{equation}
