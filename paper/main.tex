...
\section{Artist} 
The \textcolor{artist}{Artist} $\mathcal{\vartist}$\footnote{We call this transformation the artist because the Artist object in Matplotlib\cite{hunterMatplotlib2DGraphics2007} is responsible for converting data to a prerender graphical object} is a transformation from data  $\mathcal{\dtotal}$ to graphic $\mathcal{\gtotal}$.
...
We formally define data and graphic as the categories $\mathcal{\dtotal}$ and $\mathcal{\gtotal}$. To explain these categories, we first define the category of open sets $\mathcal{\dbase}$ and the category of fiber spaces $\mathcal{\dfiber}$. 

\begin{definition} The category $\mathcal{\dbase}$ consists of 
  \begin{itemize}
...
\begin{definition} For each open set object $U_{i}$ of category $\mathcal{\dbase}$, there is a set $\Gamma(U_i, \dtotal)$. The elements of this set are the sections $\{\tau^{j}:U_i \rightarrow \dfiber_{U_{i}}\}_{j\in I}$ of \dtotal\ over open set $\U_{i}$. The sections $\Gamma(\dbase, \dtotal)$ are called the global sections of $\dtotal$. \cite{SheafMathematics2021,spanier1989algebraic}.
\end{definition}
...
\begin{definition} The fiber space category $\mathcal{\dfiber}$ is a category of sets \textbf{Set}. This means its objects are sets, and its morphisms are total functions\cite{barrCategoryTheoryComputing}
\end{definition}


\begin{definition} The monad $T$ is [a,b,c]
