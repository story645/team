...
\begin{equation}
  \label{eq:actc:sheaf}
 \sheafc: \textcolor{source}{\mathcal{C}^{op}} \rightarrow \setc
\end{equation}
...
\begin{equation}
  ...
  \end{tikzcd}
\end{equation}
As shown in \autoref{eq:atct:pullback-pushforward}, the pushforward $f_*\sheaf_{X}$ modifies the sheaf $\sheaf_{X}$ such that sections $s \in \sheaf_{X}(X)$ can be evaluated over points in $y$
\begin{equation}
  \label{eq:atct:pushforward-section}
  (s: x \rightarrow \setb) \xrightarrow{f_*} (s_*: y \rightarrow \setb)
\end{equation}
while the pullback functor $f^*$ modifies the sheaf  $\sheaf_{Y}$ such that sections $\t \in \sheaf_{Y}(Y)$ can be evaluated over points in $x$
\begin{equation}
  \label{eq:atct:pullback-section}
  (t^*: x \rightarrow \setb) \xrightarrow{f^*} (t: y \rightarrow \setb)
\end{equation}
As shown in \autoref{eq:atct:pushforward-section}, the pushforward modifies the signature of the functions returned by the functor , while in \autoref{eq:atct:pullback-section} the pullback modifies the signature of the argument to the functor. 
