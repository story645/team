...
\section{Artist} 
The \textcolor{artist}{Artist $\mathcal{\vartist}$} is a transformation from data space to graphic. 
...
To show that the artist is a natural transformation, we first define the categories $\mathcal{E}$ and $\mathcal{H}$. In order to define these categories, we introduce the category of open sets $\mathcal{K}$

\begin{definition} The category $\mathcal{K}$ consists of 
  \begin{itemize}
...
The open set objects $U_{i}$ are used to track subsets of the data defined globally on \dbase. For example, 


\begin{definition} For each open set object $U_{i}$ of category $\mathcal{K}$, there is a set $\Gamma(U_i, \dtotal)$. The elements of this set are the sections $\{\dsection^{0},\cdots, \dsection^{i}, \dsection^{n}\}$ of \dtotal\ over open set $\U_{i}$. The sections $\Gamma(\dbase, \dtotal)$ are called the global sections of $\dtotal$. \cite{SheafMathematics2021,spanier1989algebraic}.
\end{definition}

Each section is how we represent a unique dataset, where the set of sections $\Gamma$ is the set of all datasets that have the same data fields \dfiber\ and continuity \dbase\. For example, database tables that have the same schema and discrete index \cite{spivakSIMPLICIALDATABASES}.


\begin{definition} The category $\mathcal{\E}$ consists of 
  \begin{itemize}
    \item\textit{objects} the presheaf $F: 
