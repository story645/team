...
\subsubsection{Topology: Base Space \dbase}
...
We encapsulate the notion of a programmatic topology type as the base space category $\mathcal{\dbasec}$. The base space category $\mathcal{\dbasec}$ has open set objects $\opensetc$. The morphisms of the category are inclusions $\iota: \opensetc_1 \rightarrow \opensetc_2$ such that $\opensetc_1 \subseteq \opensetc_2$. By definition of a catagory\cite{barrCategoryTheoryComputing}, morphisms are commutative $(\iota_{1,2}: \openset_{1} \rightarrow \openset_{2}) \circ (\iota_{2,3}: \openset_{2} \rightarrow \openset_{3}) = \iota_{1,3}: \openset_{1} \rightarrow \openset_{3}$. Another property of morphisms is that they are associative such that $\iota_{3,4}\circ(\iota_{2,3}\circ\iota_{1,2}) = (\iota_{3,4} \circ \iota{2,3})\circ \iota_{1,2}$ where $\iota_{3,4}:\openset_{3}\rightarrow \openset_{4}$. Each object also has an identity morphism $1_{\openset_i}: \openset_i \rightarrow \openset_i$. We propose that these categorical types can be mapped to 
programmatic types that describe the data topology and that the inclusion morphisms can be used to assert that the continuity has been preserved when defining subsets of data. 

\subsection{Field Types: Fiber Space \dfiber}
