...
In this section, we describe how structure has traditionally been discussed in visualization and mathematics and encapsulated in visualization library design because we propose that by formally incorporating mathematical definitions of equality, we provide a general framework that can account for a larger variety of fields than the existing work has, can formally expresse what it means to preserve the topology of the data, and we can do so in a uniform manner using category theory because that can lead to more consistent software interfaces in visualization software libraries \cite{wielsManagementEvolvingSpecifications1998,yorgeyMonoidsThemeVariations2012}.


Generally, preserving structure means that a visualization is expected to preserve the $field$ properties and $topology$ of the corresponding dataset:
...
and invertable}. A function that preserves structure when the input or output is changed by a group action is called \textit{equivariant}. Given a group $G$ that acts on both the input $X$ and the output $Y$ of a function $f: X \rightarrow Y$
...
\end{figure}
For example, although the Steven's scales were conceptualized as having group structure, the ordinal scale have a monoidal structure because partial orders ($\geq, \leq$) are not invertable. This means we cannot use \textit{equivariance} to test for structure preservation instead we use \textit{homomorphsim} because it imposes fewer constraints on the underlying mathematical structure of the data.
...
\end{definition}
which means that the operators $\circ$ and $*$ are compatiable.  \note{check grimalidi for citation}.
...
As shown in \autoref{fig:def:equivariance} and \autoref{fig:def:homomorphsim}, the basis of asserting structure preservation is that there exist compatiable actions and these actions commute; we generalize compatability and commutability as the basis for structure preservation in \autoref{sec:artist:equivariant:data}.
...
A brief visualization oriented introduction to category theory is in Vickers et al \cite{vickersUnderstandingVisualizationFormal2013}, but they are applying category theory to semantic concerns about visualization design rather than library architecture.
...
This project has been made possible in part by grant number 2019-207333 and from the
\note{Cycle 3 grant #} Chan Zuckerberg Initiative DAF, an advised fund of Silicon Valley Community Foundation
