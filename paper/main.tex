...
\keywords{Visualization software design, visualization theory, categorical framework}
\begin{document}
% make the title area
...
\begin{description}
  \item[\textcolor{fiber}{\textbf{field}}]\footnote{In this paper, color in definitions, equations, and visualizations is used to group conceptually related terms\cite{headMathAugmentationHow2022}, for example \textcolor{fiber}{field} and \textcolor{fiber}{fiber}.} is a set of values of the same type, e.g. date, location, stations, and precipitation in a weather station dataset.
  \item[\textcolor{base}{\textbf{connectivity}}] is how elements are arranged in a dataset\cite{wilkinsonGrammarGraphics2005}, e.g. as 0D ($\bullet$) points, along a 1D (--) line, in a 2D ($\blacksquare$) surface, as a 3D trajectory, a volumetric cube, or as a network.  
\end{description}

This paper builds on how structure has traditionally been discussed in visualization and mathematics and encapsulated in visualization library design to propose a uniform interface for encoding structure that supports a broader variety of fields and more rigorously defines how connectivity is preserved. Bertin\cite{bertinSemiologyGraphicsDiagrams2011} codified the idea that properties of data and graphics should match, using the Steven's measurement scales \cite{stevensTheoryScalesMeasurement1946} as the case study. Mackinlay and Wilkinson translate algebras on data to grammars for specifying graphics to develop frameworks for designing structure preserving visualization tools \cite{mackinlayAutomatingDesignGraphical1986,mackinlayAutomaticDesignGraphical1987,wilkinsonGrammarGraphics2005}, while our framework uses the same abstractions for data and graphics. Bartonicek built a tool based on the categorical and algebraic properties of stacked, dodged, and layered bar charts and the underlying data \cite{bartonicekNoMoreNo2025}; our work provides a more general framework for describing those properties and propose that sheaves are a better abstraction for reasoning about connectivity constraints \autoref{sec:atct:sheaves,sec:artist:anim_and_interactive} than monoids and groups. Similarly to Kindlemann and Scheidegger's algebraic visualization design (AVD) framework \cite{kindlmannAlgebraicProcessVisualization2014}, our framework presents a uniform abstract data representation layer in \autoref{sec:atct:sheaves} for ensuring that the visualization should not change if the data representation (i.e. the data container) changes, defines the conditions under which data is mapped unambiguously to visual encodings \cite{ziemkiewiczEmbeddingInformationVisualization2009} in \autoref{sec:atct:xi}, and provides a methodology for verifying that changes in data should correspond to changes in the visualization in \autoref{sec:artist:equivariant:artist} that does not necessarily require that the changes be perceptually significant. Our model generalizes previous mathematically derived frameworks by allowing arbitrary structures, explicitly incorporating topology, and by providing an explicit framework for translating the theoretical ideas into buildable components in \autoref{sec:construction}. 
...
\begin{equation*}
  \textcolor{section}{\texttt{dataset}}: \textcolor{base}{\texttt{connectivity}} \rightarrow \textcolor{fiber}{\texttt{fields}}
\end{equation*}

which can then be checked by visualization algorithms to ensure that the assumptions of the data match the assumptions of the algorithm. This typing system also extends to pre-rendered graphic output, allowing us to develop the structure preservation framework in \autoref{sec:artist} that ensures that output fields are equivalent to the input fields and that the connectivity of the output graphic matches the connectivity of the input.
...
Building on Spivak's constructions: 
\begin{definition}
the \textcolor{fiber}{fiber space} \dfiberc\ is the space of all possible data records:
\begin{equation}
...
As with the base space category $\mathcal{\dbase}$, we propose a fiber category $\mathcal{\dfiberc}$ to encapsulate the field types of the data. The fiber category has a single object $\dfiberc$ of an arbitrary data type (e.g. $\texttt{int}$) and morphisms on the fiber object $\dfunctc \in Hom(\dfiberc, \dfiberc)$. We can also equip the category with any operators or relations that are part of the mathematical structure of the field type, for example an addition or comparison operator. A single object category is a monoid and therefore also equipped with a bifunctor $\otimes: dfiber\times\dfiber \rightarrow \dfiber$. 

The bifunctor can be defined to express constraints on composite types. For example, dates can be represented as three fields $\dfiber_{year} \times \dfiber_{month} \times \dfiber_{day}$ or a composite fiber field $\dfiber_{year} \times \dfiber_{month} \times \dfiber_{day} = \dfiber_{date}$. The $\otimes$ encapsulates both the sets of values associated with each fiber $\{y \in \mathbb{I}\vert 1992 \leq y \leq 2025\} \times \{m \in \mathbb{I}\vert 1 \leq m \leq 12 \} \times \{d \in \mathbb{I}\vert 1 \leq d \leq 31 \}$ and the composition function $ \otimes: \dfiber_{year} \times \dfiber_{month} \times \dfiber_{day} \rightarrow \dfiber_{date}$ which would include a constraint to only return dates that have the right number of days for each month. The bifunctor also composes the morphisms associated with each category into a morphism on the composite category $(\dfunct_{year}, \dfunct_{month}, \dfunct_{day}) = \dfunct_{date}$. 
...
We encode data as a \textcolor{section}{section} \dsectionc\ of a bundle because this allows us to incorporate the topology and field types in the data definition. 

We define section functions locally, meaning that 
\begin{definition}
...
\end{equation}
such that $\dsection|_{\openset} = \dfuncpull\dsection|_{\openset^{\prime}}$ because $\dsection|_{\openset} = \dsection|_{\dfunch(\openset^{\prime})}$. This means that the base space transformation  $\dfunch(\dbasepoint^{\prime}) = \dfunch(\dbasepoint)$ such that
\begin{equation}
...
\hline
  \end{tabular}
  \caption{Example of testing whether a data container supports inclusion/subsetting in a manner that satisfies sheave constraints.}}
\end{table}
...
\label{sec:artist:homeomorphism}
...
This expectation of using the same encoding for the same variable is a generalization of the concept of consistency checking of multiple view encodings discussed by Qu and Hullman \cite{quKeepingMultipleViews2018}. This expectation can also be used to check that a multipart glyph is assembled correctly. For example, a box plot \cite{wickham40YearsBoxplots2011} typically consists of a rectangle, multiple lines, and scatter points; therefore a boxplot artist $\vartist_{boxplot} = \vartist_{rect} \times \vartist_{errors} \times \vartist_{line} \times \vartist_{points}$ must be constructed such that all the sub artists draw a graphic at or around the same x value.
...
This interface provides a uniform way of accessing subsets of the data, which are local sections. The motivation for a common data interface is that it would allow the artist to talk to different common python data containers, such as numpy\cite{harrisArrayProgrammingNumPy2020}, pandas\cite{rebackPandasdevPandasPandas2020}, xarray \cite{hoyerXarrayNDLabeled2017}, and networkx\cite{hagbergExploringNetworkStructure2008}. Currently, data stored in these containers must be unpacked and converted into arrays and matrices in ways that either destroy or recreate the structure encoded in the container. For example a pandas data frame must be unpacked into its columns before it is sent into most artists and continuity is implicit in the columns being the same length rather than a tracked base space $\dbase$. Because it is more efficient to work with the data in column order, we often project the fiber down into individual components. As shown in \autoref{eq:atct:fiber_product}, we can verify that this projection is correct by checking that the values at the index are the same regardless of the level of decomposition.
...
\acknowledgments{
The authors would like to thank the anonymous reviewers who gave constructive feedback on various versions of this paper. The authors are also grateful to the various Matplotlib and Napari contributors, particularly Juan Nunez-Iglesias, and Nicolas Kruchten for their valuable feedback from the library developer perspective. Hannah is also very grateful to Nicolas for the suggestion of augmented notation and to the nlab and wikipedia contributors who wrote clear explanations of many of the topics discussed in this paper. This project has been made possible in part by EOSS cycles 1 and 3 grants from the Chan Zuckerberg Initiative DAF, an advised fund of Silicon Valley Community Foundation. 
%grant number 2019-207333 and {Cycle 3 grant number}
}
