...
We propose that the artist $\mathcal{\vartist}$ is a function 
...
We use a fiber bundle because it allows us to generalize implementation details. In this work, \gtotal\ assumes the the display is an idealized 2D screen, but there is no barrier to encoding for example a 3D print surface. In the same vein, \gsection\ is an abstraction of rendering; it can be a specification such as PDF\cite{bienz1993portable}, SVG\cite{quintScalable2003}, ... or a rendering engine such as cairo or agg, 
