...
The mobius band is an example of a non-trivial bundle because the twist in the middle, as shown in \autoref{fig:atct:nontrivialbundle}, means that there is at least one local . 

But, as with all bundles, the mobius band can be decomposed into trivial bundle components, here the two sides of the mobius band. Each side has fibers going in the same direction, with the mismatch only apparent on the edges. These trivial bundles can be glued into the mobius band so long as the fibers (arrows on the edges) go in the same direction when glued. The data is encoded as the section of the bundle, here the sin curve. 
