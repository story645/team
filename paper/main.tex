...
\IEEEPARstart{V}{isualization} is a structure preserving mapping from data to graphic, which loosely means that the visualization retains how the data values are positioned relative to each other and the mathematical properties of those values. One way of verifying that the structure is preserved is to attempt to distill
the data set from the graphic, i.e. check that the values read off a bar chart are equivalent to the values used to construct the chart. Structure preservation assumes that the $data\leftrightarrow graphic$ mapping is bidirectional, meaning changes to either data or graphic are expected to be reflected in the other\cite{kindlmannAlgebraicProcessVisualization2014}.
 
 In this paper, we propose a methodology for describing structure, verifying structure preservation, and specifying the conditions for constructing a structure preserving map between data and graphics using topology and category theory.
 

 We use category theory because it is the math of structures and their relations and therefore provides a robust framework for defining structure and expressing the conditions under which structure would be preserved. 
 
 We encode the structure of the data and graphics in a uniform generalizable manner ; doing so allows us to discuss everything from lines to complex ND spaces with the same interface. 
