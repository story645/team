...
A topology is a partitioning of a space into sets of its subspaces and which connectivity is encoded in the topology is determined by the way in which the space is partitioned. For example, the GHCN\cite{lawrimoreGlobalHistoricalClimatology2011} dataset in \autoref{fig:related-work:continuity:ktypes} is a complex spatio-temporal record of weather station data. One way of encoding that each record is independent is to encode the underlying topology as a set of discrete points, one for each record. Each of these discrete points has a corresponding marker in the scatter plot. Another way of subsetting the data is as sets of timeseries, which can be encoded as intervals since weather station data is sampled from 1D continous natural phenomena. Each line plot can be thought of as a function evaluated on the underlying interval that encoding the temporal continuity. A third way to view the data is as a sparse sampling of a 2D geospatial grid, where the connectivity can be modeled as a 2D plane. Here the connectivity is preserved by plotting the stations in the map such that their relative distances are preserved.

note{add summary and transition}
