...
Functors preserve the domains, codomains, composition, and identities\cite{riehlCategoryTheoryContext}. Specifically, functors allow us to express which structure must be preserved when an object is transformed from one category to another. In this work, we use a presheaf functor to encode the constraints that ensure that the topological structure has been reserved. 
...
As introduced in \autoref{sec:related-work:equivariance}, data and the corresponding visual encoding are expected to have compatible structure. This structure can be formally expressed as actions $\dfunc \in \dfuncset$ on the sheaf $\sheaf_{\dbase, \dtotal}$. 
\note{rewrite for generalities probably}
\begin{definition} 
  An \textbf{action}  $act: \dfuncsetc \times \sheaf_{\dbase, \dtotal} \rightarrow \sheaf_{\dbase, \dtotal}$ is a function that expresses that elements of set $\dfuncset$ act on elements of the set of sections $\sheaf_{\dbase, \dtotal}$. Actions are associative such that 
  \begin{equation}
...
\begin{table}[h!]
  ...
  \begin{tabular}{|r c c c l|}
    \hline
 Topological Space  & Data  & Visual  & Graphic  & Encodes \\
   \hline
\textcolor{total}{total space} & \dtotal & \vtotal & \gtotal & fields + topology \\
\textcolor{base}{base space} & \dbasec & \dbasec & \gbasec & topological structure\\
\textcolor{base}{open set} & \opensetc & \opensetc & \opensetgc & subspace \\
\textcolor{fiber}{fiber space} & \dfiberc & \vfiberc & \gfiberc  & field structure\\
\hline
  \end{tabular}
  \caption{Topological spaces introduced in \autoref{sec:atct:fiber-bundles} and \autoref{sec:construction:vtotal}}
...
\begin{table}[h!]
  ...
  {\renewcommand{\arraystretch}{1.2}
  \begin{tabular}{|r c c c l|}
     & topological structure & subspace & field structure &  fields + topology\\
     & \textcolor{base}{total space} & \textcolor{base}{openset} & \textcolor{fiber}{field types} && 
  \end{tabular}
  \caption{Topological spaces introduced in \autoref{sec:atct:fiber-bundles} and \autoref{sec:construction:vtotal}}
  \label{tab:appendix:summary:objects}
\end{table}
...
\textcolor{section}{section} & $\dsectionc: \dbase \textcolor{section}{\rightarrow} \dfiberc$ &  $\vsectionc: \dbase \textcolor{section}{\rightarrow} \vfiberc$ & $\gsectionc: \gbase \textcolor{section}{\rightarrow} \gfiberc$ & data set \\
\textcolor{sheaf}{sheaf} & $\sheafc_{\dbasec, \dtotalc}: \opensetc \rightarrow \cgamma{\opensetc}{\dtotalc\restriction_{\opensetc}}$ & $\sheafc_{\dbasec, \vtotalc}: \opensetc \rightarrow \cgamma{\opensetc}{\vtotalc\restriction_{\opensetc}}$ & $\sheafc_{\gbasec, \gtotalc}: \opensetgc \rightarrow \cgamma{\opensetc}{\gtotalc\restriction_{\opensetgc}}$ & subspace to records map \\ 
\hline
  \end{tabular}
  \caption{Functions that associate topological subspaces with records. Equivalent constructions can be applied to elements in visual and graphic sheaves, and these functions are distinguised through subscripts $\dfunc_{\dtotal}$, $\dfunc_{\vtotal}$ and $\dfunc_{\gtotal}$}
...
\centering
{\renewcommand{\arraystretch}{1.2}
\caption{Functions $\dfunc=(\dfunchc, \dfunctc)$ for modifying data records}
\label{tab:appendix:summary:datamod}
\end{table}


\note{color and replace source/target with constraint}
\begin{table}[h!]
  \centering
\begin{tabular}{|r|l|l|}
  \hline
  ...
  \hline
\end{tabular}
\caption{artist, verification functions, and construction $\vartist = \vmarkc \circ \nu$ introduced in \autoref{sec:atct:xi}, \autoref{sec:artist}, and \autoref{sec:construction}}
