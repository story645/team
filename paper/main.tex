...
\begin{figure}[h!]
  ...
  \caption{
    A topological representation of the data (in purple) is a simplified representation of how the records in a dataset are connected. The table is a set of independent records; therefore it has a 0D continuity represented by the disconnected dots. These records can also be samples from a timeseries, which has a 1D continuity. \note{started making a map of the temperature data but was taking a while so I didn't  }
    \label{fig:related-work:continuity:ktypes}}
\end{figure}

 

Roughly, topology is a formal way of encoding the connectivity of the elements of the dataset \cite{wilkinsonGrammarGraphics2005} and there is an expectation that the visual elements should match this connectivity. Formally, a topological space is a set $X$ with a structure called a topology. The set $X$ is a collection of points, where these points are \note{getting tripped up on how much of the definition to include here, but this is the natural place to give a definition of topology and opensets}
Some common topologies, as illustrated in\autoref{fig:related-work:continuity:ktypes}, are point, lines, and surfaces. The table is an example of disconnected discrete elements and therefore has the topology of discrete points. The timeseries is  sampled from a 1d continuous temperature measurement, and therefore has a topology of a 1D space. The map in \autoref{fig:related-work:continuity:ktypes} shows a 2D continuous surface. As illustrated in the figure, it is not uncommon for the same dataset to have multiple topologies. For the common topologies of discrete points, lines, and surfaces, Bertin recommend that they be represented visually by point, line, and area type visual elements respectively \cite{bertinSemiologyGraphicsDiagrams2011a}, and this expectation has been generalized in the development of visual algoriithms. 

 

Visual algorithms assume the continuity of their data, as described in taxonomies of visualization algorithms Chi and by Troy and M\"{o}ller \cite{chiTaxonomyVisualizationTechniques2000,toryRethinkingVisualizationHighlevel2004} and try to preserve it, but many data structures do not provide a method for encoding the topology of the data they store. For example, a \texttt{line} algorithm often take as input a list of (x,y) coordinates. The \texttt{line} algorithm assumes that its input is continuous and therefore displays the data as if it was continuous, regardless of the actual data topology. The ability to encode the continuity expectations in the visual algorithm and the data container would provide a way of verifying that the assumptions match and therefore that the components are preserving the topological structure. 
