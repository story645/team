...
\begin{equation}
  ...
  \begin{tikzcd}
    ...
    \opensetgc_i \arrow[uu, "\sheafc(\gtotalc)"] \arrow[rruu, "\vartistc(\sheafc(\dtotalc))\coloneqq \sheaf_{\vartist}(\dtotalc)" description, dashed] &  & 
    \end{tikzcd}
\end{equation}
...
\end{equation}
can be used to map between open sets $\openset_i \subset \dbase$ in data space and the open sets in visual space $\opensetg_i \subset \gbase$. 
...
We compose the functor $\vindex_*$ with the sheaf $\sheaf$ to move the sets of sections from $\Gamma(\openset, \dtotal)$ to $\Gamma(\opensetg, \vindex_* \dtotal)$. As illustrated in \autoref{fig:artist:pushforward_xi}, this operation copies a section and a fiber over one point $\dbasepoint \in \dbase$ such that it repeats over the corresponding region $\gbasepoint \subset \gbase$ in the graphic space. 
