...
A fiber bundle $(\dtotalc, \dbasec, \pi, \dfiberc)$ is a structure with topological spaces $\dtotalc, \dfiberc, \dbasec$ and continuous surjective map $\pi: \dtotalc \rightarrow \dbasec$ \cite{FiberBundle2020}. 
\begin{equation}
...
\end{equation} 
 ...
 $\textcolor{set}{\Gamma}$ denotes the set of all sections. The set of sections $\dsection$ of a bundle are continuous functions 
\begin{equation}
...
We encode the underlying connectivity of the data as the \textcolor{base}{base space} \dbase\ of the fiber bundle because it provides a uniform language for describing the topology regardless of how the data is subset. The base space \dbase\ is a topological space $(\dbase,\mathcal{T}_{\dbase})$, which means it consists of the set of points ${\dbasepoint \in dbasec}$ and topology $\mathcal{T}_{\dbase}$. A topology $\mathcal{T}_{\dbase}$ is a collection of open sets $\openset$ such that the empty set and \dbase are in $\mathcal{T}_{\dbase}$,and every union and intersection of opensets in $\mathcal{T}_{\dbase}$ are also in $\mathcal{T}_{\dbase}$ \cite{bradleyTopologyCategoricalApproach2020}. An openset \openset\ is a generalization of an open interval to a region, meaning an open set contains points inside a bounded region on \dbase and excludes the points on the boundary \cite{TopologyVsTopology}. A property of the base space \dbase\ is that it is a quotient topology, ,meaning it has the finest topology of $\dbase$, which  means that it contains the largest possible collection of distinct open sets on $\dbase$ \cite{QuotientSpaceTopology2020}. For our purposes, $\dbase$ is a set of points that encodes the underlying connectivity of the our dataset, for example, $\dbase$ could be the discrete points, interval, or plane in \autoref{fig:related-work:continuity:ktypes}. The topology on $\dbase$ encodes the space of smallest possible subsets of the data, and the union and intersection rules express that these subsets must be combinable into larger subsets of the data. 

We propose that we can express topological structure programmatically by  constructing data types that encapsulates the class of topological structure-.i.e point, line, plane, network, etc. These data types can be formally specified as objects of a category $\mathcal{\dbasec}$. 
The base space category $\mathcal{\dbasec}$ has open set objects $\opensetc$. The morphisms between objects of the category are inclusions $\opensetc_i \xrightarrow{\iota} \opensetc_j$ such that $\opensetc_i \subseteq \opensetc_j$. Given two morphisms $\opensetc_i \xrightarrow{\iota_{i,j}} \opensetc_j$ and $\opensetc_j \xrightarrow{\iota_{j,k}} \opensetc_j$, these morphisms must compose 
\begin{equation}
  \label{eq:atct:base:cat:comp}
\begin{tikzcd}
  \openset_{i} \arrow[rr, "{\iota_{i,j}}"] \arrow[rrrr, "{\iota_{i,k}}", bend right] &  & \openset_{j} \arrow[rr, "{\iota_{j,k}}"] &  & \openset_{k}
\end{tikzcd}
\end{equation}
such that there exists a function $\opensetc_i \xrightarrow{\iota_{i,k}} \opensetc_k$.  The expected composition of functions is often expressed as commutative diagrams, such as \label{eq:atct:base:cat:comp}. Compositions of morphisms are associative
\begin{align}

 & ((\openset_{i}\xrightarrow{\iota_{i,j}} \openset_{j})) \xrightarrow{\iota_{j,k}} \openset_{k} \xrightarrow{\iota_{k,l}} \openset_{l}\label{eq:atct:cat:as1}
  \\
  = & ((\openset_{j}\xrightarrow{\iota_{j,k}} \openset_{k})\xrightarrow{\iota_{k,l}} \openset_{l})

      
    \openset_{i}\xrightarrow{\iota_{i,j}} \openset_{j}\label{eq:atct:cat:as2}
\end{align}
\note{this needs to be sorted cause this format ends up w/ the same order}

such that \autoref{eq:atct:cat:as1} and \autoref{eq:atct:cat:as2}  return the same value. In general associativity allows us flexibility in building call stacks of curried functio


 While here we discuss composition, associativity, and identity in the context of $\mathcal{\dbase}$, these are general properties of categories \cite{riehlCategoryTheoryContext,barrCategoryTheoryComputing,fongInvitationAppliedCategory2019}. 





\subsection{Data Fields: Fiber Space \dfiber}
We encode the field types of a dataset as the  \textcolor{fiber}{fiber space} \dfiberc of the fiber bundle because it allows us to encode fields of varying types and dimensionalities in a consistent manner. The fiber \dfiber\ is the preimage of the projection function $\pi$ at a point in the base space $\dfiber_{\dbasepoint} = \pi^{-1}(\dbasepoint)$, meaning that all points in a fiber space $\dfiber_{\dbasepoint}$ map to the same point $\dbasepoint$. The fiber space can be used to encode the space of all possible values of a data field and the fibers can be named, as described by Spivak \cite{spivakDatabasesAreCategories2010,spivakSIMPLICIALDATABASES}. The space of all possible values is analogous to a CS data type; therefore the fiber space can be used to encode the data types of the data fields. The fiber is also a topological space, meaning there is no restriction on the dimensionality of the field, thus allowing for the encoding of multi-dimensional fields in the same manner as single dimensional fields. 

As with the base space category $\mathcal{\dbase}$, we propose a fiber category $\mathcal{\dfiberc}$ to encapsulate the field types of the data, which is akin to a schema in a database \cite{ullmanFirstCourseDatabase2008}. The fiber category $\mathcal{\dfiberc}$ is a monoidal category, meaning the category has a single object $\dfiberc$ of an arbitrary type. The morphisms on the fiber object are $\dfunctc \in Hom(\dfiberc, \dfiberc)$. The fiber category is equipped with a bifunctor $\otimes: \mathcal{\dfiber} \times \mathcal{\dfiber} \rightarrow \mathcal{\dfiber}$. The bifunctor provides a method for combining fibers, thereby allowing for fields that contain multityped values. For example, wind can be represented as two fields $\dfiber_{speed} \times \dfiber_{direction}$ or a composite fiber field $\dfiber_{speed} \otimes \dfiber_{direction} = \dfiber_{wind}$. The $\otimes$ encapsulates both the sets associated with each fiber $\mathbb{R} \times \mathbb{R} = \mathbb{R}^{2}$ and the morphisms associated with each functor $(\dfunct_{speed}, \dfunct_{direction}) = \dfunct_{wind}$. Defining the field schema as a monoidal category allows us to formally express the structure of the field, as defined by its sets and functions, at different levels of composition as needed by different visualization tasks.

\subsection{Data: Section \dsection}
We encode data as a \textcolor{section}{section} \dsectionc\ of a bundle because this allows us to incorporate the topology and field types in the data definition. Sections  $\dsection: \dbasepoint \mapsto \delement$ map from a point in the basespace $\dbasepoint \in \dbase$ to a record in the fiber space $\delement \in \dfiber_{\dbasepoint}$. We can therefore model data as structures that map from an index like point $\dbasepoint$ to a data record $\delement$. Bundles can have multiple sections, as denoted by $\Gamma(\openset, \dtotal\restriction{\openset})$ which encapsulates the notion of multiple datasets with a shared fiber and base space. For example, all functions on the interval [0, 1] or a set of database tables with the same schema. 
...
\{\textcolor{section}{\texttt{data-subset}}: \textcolor{base}{\texttt{topology}} \rightarrow \textcolor{fiber}{\texttt{fields}} \\
 s.\;t.\;\textcolor{section}{\texttt{data-subset}} \in \texttt{dataset}\}  
  \end{split}
\end{equation}
that can be glued together on overlaps to generate the dataset. We discuss this gluing in \autoref{sec:atct:sheaves}.



\subsection{intrepretation/example}
\begin{figure}[h!]
  \includegraphics[width=\columnwidth]{fbnet.png}
  \caption{\label{fig:atct:networkbundle}}
\end{figure}
\note{figure out picture/example}
Extending Spivak's fiber bundle description of a table, we define the values returned by a section as named and typed records. Since the fiber space is a topological space, it does not really have restrictions on the dimension of the types it can encode or the structure of those types. For example, wind has a speed component and a direction component and we can consider those separately $\dfiber_{speed} = \mathbb{R}, \dfiber_{direction} = \mathbb{R}$ or as one fiber$\dfiber_{wind} = \mathbb{R}^{2}$. The base space and the section evaluated on the base space form a sort of key-value pair, as described by Munzner \cite{munznerVisualizationAnalysisDesign2014}. She imbues semantic meaning to the topology, wherein the key is an independent attribute used as a look up key to retrieve the value in the dataset. She defines the value as a dependent attribute. In this work we use a more general notion of the topological space as an indexing space for the data records. For example, a data cube of temperature can be encoded such that every fiber is a set of 2D temperature surfaces $\dfiber_{tempsurf}$, the base space is a 1D continuous interval, and each section returns a surface $\dsection(\dbasepoint) = f(x,y)$ where x and y are already parameterized. Alternatively, the fiber can encode each cell of the data cube $\dfiber_{lat} \times \dfiber_{lon} \times \dfiber_{temp}$, the base space could be a 3D volume, and each section returns a $\dsection(\dbasepoint) = \{lat, lon, temp\}$ record. 
...
Presheaves are functors, meaning they are functions from objects of one category to objects of another category\cite{WhatFunctorDefinitions} that preserve the morphisms in each category. 
A presheaf is a contravariant functor from an object in an arbitrary category to an object in the category \setb\cite{nlab:presheaf}. 
\begin{equation}
...
\end{equation*}
which are the outputs of the sheaf $\sheaf(\openset) = \cgamma{\openset}{\dtotal\restriction_{\openset}}$. We use a  presheaf to describe the expectation that any set of sections defined over a larger space $\dsection \in \Gamma(\opensetc_2, \dtotal\restriction_{\openset_{2}})$ is also defined over subspaces of that space $\dsection\restriction_{\opensetc_{1}}$. 

The locality and gluing axioms \cite{bakerMathsSheaf} of a sheaf describe the conditions for verifying that the sections over subspaces are part of a cohesive section over the whole space. The locality axiom asserts that two sections in a sheaf are equal $\dsection^{a} = \dsection^{b}$ when they evaluate to the same values over the same set of open sets $\dsection^{a}|{\openset} =  \dsection^{b}|_{\openset}$. The gluing axiom asserts that the union of sections defined on specific open sets is equivalent to one big section over the union of spaces $\dsection|_{\openset_{i} \cup \openset_{j}} = \dsection^{i}|_{\openset_i} \cup \dsection^{j}|_{\openset_j}$ if these sections agree on overlaps $\dsection^{i}|_{\openset_i\cap\openset_j} =  \dsection^{j}|_{\openset_i\cap\openset_j}$ The gluing axiom generalizes to expresses an equivalency between a concatenated representation of data and distributed representation of the same data. In \autoref{fig:atct:networkbundle} this axiom means that the sections over each node and the edge can be combined into one section over the piece of the network in the figure.
...
Given the map between spaces \vindex\, a property of sheaves is that there exist image functors\cite{ImageFunctorsSheaves2021} functors \vindexpull\ and \vindexpush that can transport sheaves over other base spaces. The \textcolor{functor}{pullback functor} $\vindexpullc$ sends sheaves over $\dbase$ to $\gbase$ and the \textcolor{functor}{pushforward functor} $\vindexpushc$  sends sheaves over $\gbase$ to $\dbase$
\begin{equation}
