...
Tests of structure preservation, such as equivariance and homomorphism (Supplement~\autoref{supp-sec:equiv, supp-sec:homo}) verify that actions commute such that the order in which they applied does not affect the output. In \autoref{fig:actions}, the action $\dfunch$ permutes the indexing space such that location of the bars relative to each other changes. The action $\dfunct$ doubles the data value associated with the height of the bar. Because these are both structure preserving actions, the order in which the bars are transformed - permute then scale or scale then permute - does not effect the final image. This means that structure preserving library components should also not be sensitive to the order of transforms.  
...
\begin{definition}\label{def:atct:presheaf}
  A \textbf{presheaf} $F:\mathcal{C}^{op} \rightarrow \setb$ is a contravariant functor from an object in an arbitrary category to an object in the category \setb\cite{spanierAlgebraicTopology1989}.
\end{definition}
...
\begin{definition}\label{def:atct:sheaf}\cite{bakerEuclideanSpaceMathsSheaf,spanierAlgebraicTopology1989} A \textbf{sheaf} is a presheaf that satisfies the following two axioms
\begin{itemize}
