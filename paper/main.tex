...
\note{The mapping between data and visualization is expected to be bidirectional -> that's what we really mean by equivariance and homomorphism  }
\IEEEPARstart{V}isualizations are expected to be structure preserving, meaning that when data is transformed into a graphical representation enough of the structure of the data is present in the graphic that the data can be extracted from the graphical representation. Changes to structure through changing the data or the graphic in the $data\leftrightarrow graphic$ mapping are expected to be reflected in each inputs corresponding output \cite{kindlmannAlgebraicProcessVisualization2014}.Because visualizations are assumed to preserve structure, visual algorithms implemented in visualization producing tools, such as software libraries, are also expected to preserve structure; frequently these expectations are either codified implicitly, only for some aspects of the data structure, or inconsistently with respect to visualization algorithm type. This means that currently many visualization tools lack an explict, comprehensive, and consistent way to express what conditions must be met to gaurantee that the component produced a structure preserving visualization. 


We propose that we can encode the structure of the data and graphics in a uniform generalizable manner by using a mathematical structure from algebraic topology called a fiber bundle, and that we can use category theory to explicitly specify what it means for a visualization tool component to preserve structure. These algebraic structures can be translated into analogous programmatic types for encoding structure, while the use of category theory facilitates developing a functional design framework that describes function composition in terms of functional algebraic operations. The typing system and composition inherent in a functional design encourage library developers to build complex visualization components from simple verifiable parts that have few side effects\cite{huHowFunctionalProgramming2015, hughesWhyFunctionalProgramming1989}. These categorically specified components could be built as a standalone library and integrated into existing libraries because they are inherently self-contained. Furthermore we see the application of these ideas in low level visualization library design as our motivation, and we hope these ideas will help shape the reorganization of critical data visualization libraries, such as Matplotlib. The contribution of this paper is a mathematical framework for describing structure preserving visualization components in a manner that is generalizable to different types of inherent structure. This framework also provides guidance for the construction and testing of structure preserving visualization library components.
