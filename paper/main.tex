...
\begin{definition} An \textbf{action}  $\act: \dfuncsetc \times \dtotalc \rightarrow \sheaf_{\dbase, \dtotal}$ such that 
  \begin{equation}
    act(\dfuncc_{j}, act(\dfuncc_{i}, \dsectionc)) = act(\dfuncc_{j} \circ \dfuncc_{i}, \dsectionc)
  \end{equation}
where $\dfunc_{i}, \dfunc_{j}$ are elements of the set $\dfuncset$ equipped with operator $\circ$ and \dsection\ is an element of the sheaf  $\sheaf_{\dbase, \dtotal}$.\cite{nlab:action}
\end{definition}.
