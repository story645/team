...
We model data as the sheaf $\sheaf_{\dtotal}: \mathcal{\dbase}^{op}\rightarrow \mathcal{\dtotal}$
\begin{equation}
...
\begin{definition} 
  ...
  The fiber category $\mathcal{\dfiber}$ consists of 
  \begin{itemize}
    \item \textit{object} a single object \dtotal, where \dtotal\ can be an object of any arbitrary category $\mathcal{C}$ 
    \item \textit{morphisms} $\dfunc \in hom_{\mathcal{F}}(F,F)$
\end{definition}

The fiber space over a point is embedded in the stalk over that point $\dfiber_{k}\hookrightarrow \mathscr{F}_{k}$. The sheaf maps \dfunc\ restricted to the stalk are members of $hom_{\mathcal{F}}(F,F)$. The maps $\varphi \in Hom_{\mathcal{\dfiber}}(\dfiber,\dfiber)$ include the types of data transformations described in \autoref{sec:related-work:equivariance}. When a fiber bundle is trivial, we can dispense with localization and directly consider the fiber maps $Hom(\dfiber_{k}, \dfiber_{k})$ over a point \dbasepoint. In \autoref{sec:artist:construction}, we assume that the fiber bundles are trivial.
